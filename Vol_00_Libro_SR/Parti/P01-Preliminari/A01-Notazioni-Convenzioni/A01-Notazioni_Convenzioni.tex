...
ambiente geometrico dell'intera trattazione.\footnote{Avendo a che 
	...
	$2$ e $3$. Se la dimensione dello spazio sarà differente da 3, verrà esplicitamente indicato.} Gli 
...
\textit{componenti} di $\vb$ nella base scelta. La tripletta ordinata delle componenti 
...
Sempre in grassetto italico verrano indicati vettori numerici di $\Ro^n$, con $n$ qualsiasi, dati da opportuno 
assemblaggio di componenti di vettori, cui evidentemente non corrisponde un vettore geometrico reale; in questo caso, le dimensioni
fisiche degli elementi di queste liste possono anche non essere omogenee. 
 La differenza sarà 
...
$f^\pm \coloneqq \lim_{x\to x_0^\pm} f(x)$. 

%Non c'è cammino, non c'è 
...
\todo[inline]{Non sono d'accordo con quanto scritto sotto. La distinzione può essere utile se una funzione dipende contemporanemente da tempo e spazio, altrimenti non ha molto senso. Una funzione con $f'(x)=0$ è COSTANTE, qualunque cosa sia $x$. Poi non siamo neanche coerenti, "costante" compare in molti diagrammi delle CdS.}
