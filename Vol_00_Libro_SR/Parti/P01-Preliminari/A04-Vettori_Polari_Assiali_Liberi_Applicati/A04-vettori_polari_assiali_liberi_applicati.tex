...
\begin{enumerate}
	\item $(\ub+\vb, \wb)=b(\ub,\wb)+b(\vb, \wb)$
	\item $(\ub, \wb+\vb)=b(\ub,\wb)+b(\ub, \wb)$
	\item $b(\alpha\ub+\vb)=b(\ub,\alpha\vb)=\alpha\,b(\ub, \vb)$
\end{enumerate} 
per ogni $\ub, \vb, \wb\in \Uc$, $\alpha\in \Ro$.

La forma bilineare $b$ è \textit{simmetrica} se 
\begin{equation}
	b(\ub, \vb)=b(\vb,\ub) \qquad \forall \ub, \vb\in \Uc
\end{equation}
...
e inoltre
\begin{equation}
	\ub\cdot\ub\geq0 \quad \Longleftrightarrow \ub=\mathbf{0}.
\end{equation}
\end{enumerate}


Dati due vettori $\ub, \vb \in \Uc$, cui associamo le liste $\bm{u},\bm{v}$, definiamo il prodotto scalare standard (o canonico) la forma bilineare
\begin{equation}
...
\begin{enumerate}
	\item $|\ub|\geq 0$, e vale l'uguaglianza se e e solo se $\vb=\mathbf{0}$
...
La disugliaglianza di Schwartz \eqref{schwartz} consente di introdurre nello spazio vettoriale $\Uc$ la nozione di angolo convesso tra due vettori non nulli, secondo
...
Il prodotto vettoriale può essere definito usando il prodotto scalare, il determinante e un'orientazione dello spazio. In particolare,
si richiede che il vettore $\ub\times\vb$ sia l'unico vettore tale che
\begin{equation}
\ub\times\vb\cdot \wb=\det (\ub, \vb, \wb), \qquad \forall\wb\in\Uc.
\end{equation}
...
Dal punto di vista geometrico, il prodotto vettoriale $\ub \times \vb$ è il vettore ortogonale al piano
