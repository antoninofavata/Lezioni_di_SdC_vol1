...
Dati due vettori $\ub, \vb \in \Uc$, cui associamo le liste $\bm{u},\bm{v}$, definiamo il \textit{prodotto scalare standard} (o \textit{canonico}) la forma bilineare
\begin{equation}
	\ub \cdot \vb = u_x v_x + u_y v_y + u_z v_z = \bm{u}^\top\bm{v}.
\end{equation}

Se $\ub, \vb \in \Uc$, allora vale \textit{disuguaglianza di Schwartz}
\begin{equation}\label{schwartz}
(\ub\cdot\vb)^2\leq |\ub\cdot\ub|\,|\vb\cdot\vb|.
\end{equation}

Se $\ub\in \Uc$, la \textit{norma}, o \textit{lunghezza}, di $\ub$ si definisce come
\begin{equation}
	|\ub|\coloneqq\sqrt{\ub\cdot\ub},
\end{equation}
che consente di scrivere la disuguaglianza di Schwartz come
\begin{equation}\label{schw2}
	|\ub\cdot\vb|\leq |\ub|\,|\vb|.
\end{equation}

La lunghezza di un vettore gode delle seguenti proprietà, per ogni $\ub,\vb\in\Uc$, $\alpha\in\Ro$:
\begin{enumerate}
	\item $|\ub|\geq 0$, e vale l'uguaglianza se e e solo se $\ub=\mathbf{0}$
	\item $|\alpha\ub|=|\alpha|\,|\vb|$
	\item $|\ub+\vb|\leq |\ub|+|\vb|$ e vale l'uguaglianza se e solo se $\ub$ e $\vb$ sono paralleli.
\end{enumerate}


\medskip

La disugliaglianza di Schwartz \eqref{schwartz} consente di introdurre nello spazio vettoriale $\Uc$ la nozione di \textit{angolo convesso tra due vettori} (non nulli), secondo
