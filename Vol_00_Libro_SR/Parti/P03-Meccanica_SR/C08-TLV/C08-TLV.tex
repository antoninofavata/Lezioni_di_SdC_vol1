...
\tR{In questo capitolo ci sono vari problemi. 
Quella che viene chiamata identità bilineare non è il PLV. Per come è scritta, vale ogni volta che $(\bm{u}, \bm{s})$ è congruente e
$(\bm{r}, \bm{f})$ è equilibrata. L'identità  si applica dunque solo a coppie che già soddisfano entrambe le condizioni.
Ora prova a usarla per derivare, per esempio, l'equilibrio da congruenza+PLV. Hai una coppia $(\bm{u}, \bm{s})$ 
 di cui sai che è congruente, e vorresti concludere che una qualche coppia $(\bm{r}, \bm{f})$ 
 è equilibrata, usando l'identità bilineare. Ma per invocarla, su quella $(\bm{r}, \bm{f})$, devi già sapere che è equilibrata. È una dimostrazione circolare.
 Nella dimostrazione si rilassa l'ipotesi che $(\bm{r}, \bm{f})$  sia bilanciato e si dice che l'identità bilineare vale anche solo per sistemi congruenti. Perché mai?
