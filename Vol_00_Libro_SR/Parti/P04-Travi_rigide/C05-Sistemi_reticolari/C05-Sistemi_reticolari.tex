...
uniforme: il taglio $T_\text{eq}$ è uniforme
...
lo sforzo normale non è uniforme lungo l'asta:
...
è uniforme e le reazioni longitudinali alle due
...
\begin{esempio}[Travatura a correnti paralleli con carico distribuito]
	...
	\begin{svolgimento}
		...
		il taglio è uniforme a tratti e cambia segno
...
%----------------------------------------------------------------------------------------------------------------------------
\section{Esercizi di sintesi}
\label{sec:esercizi_sintesi_reticolari}
%----------------------------------------------------------------------------------------------------------------------------

Gli esempi sviluppati nel corso del capitolo isolano
ciascuno un singolo strumento di analisi. Gli esercizi
che seguono hanno invece carattere di sintesi: su uno
stesso schema si combinano la classificazione, i criteri
di riconoscimento delle aste scariche, il metodo dei nodi
e il metodo delle sezioni di Ritter, scegliendo di volta
in volta lo strumento più conveniente e usando il
confronto fra metodi diversi come controprova del
risultato. La convenzione di segno resta quella adottata
nel capitolo: $N>0$ indica trazione, $N<0$ compressione.

\begin{esercizio}[Travatura a tre maglie triangolari]
	\label{ex:tre_maglie}
	
	Si consideri la travatura reticolare piana di
	\FIGref{fig:tre_maglie_schema}, costituita da
	tre maglie triangolari. Riferiti a un sistema cartesiano
	con origine nel nodo $A$, asse $x$ orizzontale verso
	destra e asse $y$ verticale verso l'alto, i nodi
	d'intradosso hanno
	coordinate $A=(0,\,0)$, $B=(\ell,\,0)$,
	$C=(2\ell,\,0)$; i nodi d'estradosso
	$D=(2\ell/3,\,\ell/3)$, $E=(4\ell/3,\,\ell/3)$.
	Le sette aste sono la catena inferiore $AB$ ed $BC$,
	i correnti inclinati $AD$ ed $EC$, il corrente
	superiore $DE$ e le due diagonali $DB$ ed $BE$.
	In $A$ è posta una cerniera, in $C$ un carrello con
	piano di scorrimento orizzontale (reazione verticale).
	Nel nodo $D$ agisce una forza orizzontale di intensità
	$2F$ diretta verso destra; nel nodo $E$ una forza
	verticale di intensità $3F$ diretta verso il basso.
	Si determinino gli sforzi normali in tutte le aste.
	...
	\begin{svolgimento}
		...
		\begin{figure}[htbp]
			...
			\resizebox{0.9\textwidth}{!}{%
				\begin{tikzpicture}[
					...
					nodo/.style={circle, fill=white, draw=black,
						inner sep=0pt, minimum size=6pt, line width=1.2pt},
					quota/.style={thin, gray,{Stealth[length=4pt,width=3pt]}-{Stealth[length=4pt,width=3pt]}},
					richiam/.style={thin, gray, dashed},
					forza/.style={->, >=stealth, line width=1.5pt, black},
					]
					\useasboundingbox (-1.6,-1.5) rectangle (7.2,2.6);
					...
					%% forza 2F orizzontale in D (verso destra)
					\draw[forza]($(D)+(-1.35,0)$)--($(D)+(-0.12,0)$);
					\node[left,font=\normalsize]at($(D)+(-1.35,0)$){$2F$};
					
					%% forza 3F verticale in E (verso il basso)
					\draw[forza]($(E)+(0,1.30)$)--($(E)+(0,0.12)$);
					\node[above,font=\normalsize]at($(E)+(0,1.30)$){$3F$};
					...
					\node[above =2pt,   font=\normalsize]at(D){$D$};
					\node[above right=0pt,  font=\normalsize]at(E){$E$};
					
					%% quota altezza l/3 (sinistra)
					\draw[richiam](A)--++(-1.1,0);
					\draw[richiam](0,1)--(-1.1,1);
					\draw[quota](-0.9,0)--node[left,font=\small]{$\ell/3$}(-0.9,1);
					
					%% quotatura orizzontale: linea unica, proiezioni di A,D,B,E,C
					\draw[richiam](A)--++(0,-1.0);
					\draw[richiam](D)--++(0,-2.0);
					\draw[richiam](B)--++(0,-1.0);
					\draw[richiam](E)--++(0,-2.0);
					\draw[richiam](C)--++(0,-1.0);
					\draw[quota](0,-0.9)--node[below,font=\small]{$2\ell/3$}(2,-0.9);
					\draw[quota](2,-0.9)--node[below,font=\small]{$\ell/3$}(3,-0.9);
					\draw[quota](3,-0.9)--node[below,font=\small]{$\ell/3$}(4,-0.9);
					\draw[quota](4,-0.9)--node[below,font=\small]{$2\ell/3$}(6,-0.9);
					
				\end{tikzpicture}
			}
			\caption{Travatura a tre maglie triangolari
				$ADB$, $DBE$, $BCE$. Geometria simmetrica
				rispetto alla mezzeria $x=\ell$; carichi non
				simmetrici: forza orizzontale $2F$ in $D$ e
				forza verticale $3F$ in $E$.}
			\label{fig:tre_maglie_schema}
		\end{figure}
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		
		\noindent\textbf{Classificazione.}
		Con $n_n = 5$ nodi, $n_a = 7$ aste e
		$m_\text{est} = 3$ (cerniera in $A$: due componenti;
		carrello in $C$: una componente), la formula di
		M\"{u}ller-Breslau dà
		\[
		\nu = 2\times 5 - 7 - 3 = 0.
		\]
		La travatura è isostatica: l'equilibrio globale
		determina le reazioni esterne e l'equilibrio nodale
		determina univocamente i sette sforzi normali.
		$\quad\checkmark$
		
		\noindent\textbf{Ispezione delle aste scariche.}
		Si applicano per primi i criteri di riconoscimento.
		I soli nodi con tre aste convergenti sono $D$ ed $E$,
		ma in entrambi è applicata una forza esterna: il
		secondo criterio, che richiede l'assenza di carico
		nel nodo, non è dunque invocabile. Il nodo $B$ ha
		quattro aste, i nodi $A$ e $C$ sono vincolati.
		Nessuna asta risulta scarica per ispezione --- un
		risultato negativo ma istruttivo, che ricorda come
		i criteri siano \emph{condizionati} all'assenza di
		carico nel nodo considerato: la stessa travatura,
		caricata diversamente, potrebbe presentare aste
		scariche.
		
		\noindent\textbf{Reazioni vincolari.}
		Le incognite esterne sono $X_A$, $Y_A$ (cerniera in
		$A$) e $Y_C$ (carrello in $C$). Dall'equilibrio
		globale:
		\[
		\sum F_x = 0:\quad
		X_A + 2F = 0
		...
		\[
		\sum \mu_A = 0:\quad
		Y_C\cdot 2\ell - 2F\cdot\frac{\ell}{3}
		- 3F\cdot\frac{4\ell}{3} = 0
		\;\Rightarrow\;
		Y_C = \frac{7}{3}F,
		\]
		\[
		...
		Y_A = \frac{2}{3}F.
		\]
		Il momento della forza $2F$ in $D$ rispetto ad $A$
		ha braccio pari alla quota $\ell/3$ del punto di
		applicazione; quello della forza $3F$ in $E$ ha
		braccio pari all'ascissa $4\ell/3$.
		
		\noindent\textbf{Metodo dei nodi: cascata sui nodi
			canonici.}
		Note le reazioni, i nodi $A$ e $C$ presentano
		ciascuno due sole aste incognite e sono quindi
		canonici. Si parte da essi e si procede verso
		l'interno. Conviene introdurre i versori dei
		correnti inclinati: $AD$ (ed $EC$ per simmetria)
		forma con l'orizzontale un angolo $\alpha$ con
		$\cos\alpha = 2/\sqrt5$, $\sin\alpha = 1/\sqrt5$;
		le diagonali $DB$ ed $BE$ sono a $45^\circ$.
		
		\smallskip
		\textit{Nodo $A$} (aste $AB$ orizzontale e $AD$):
		\[
		\sum F_y = 0:\;
		N_{AD}\,\frac{1}{\sqrt5} + \frac{2}{3}F = 0
		\;\Rightarrow\;
		N_{AD} = -\frac{2\sqrt5}{3}F
		\quad\text{(compressione)},
		\]
		\[
		\sum F_x = 0:\;
		N_{AB} + N_{AD}\,\frac{2}{\sqrt5} - 2F = 0
		\;\Rightarrow\;
		N_{AB} = \frac{10}{3}F
		\quad\text{(trazione)}.
		\]
		...
		\smallskip
		\textit{Nodo $C$} (aste $BC$ orizzontale e $EC$):
		\[
		\sum F_y = 0:\;
		N_{EC}\,\frac{1}{\sqrt5} + \frac{7}{3}F = 0
		\;\Rightarrow\;
		N_{EC} = -\frac{7\sqrt5}{3}F
		\quad\text{(compressione)},
		\]
		\[
		\sum F_x = 0:\;
		-N_{BC} - N_{EC}\,\frac{2}{\sqrt5} = 0
		\;\Rightarrow\;
		N_{BC} = \frac{14}{3}F
		\quad\text{(trazione)}.
		\]
		...
		\smallskip
		\textit{Nodo $D$} (note $N_{AD}$ e il carico $2F$;
		incognite $N_{DB}$ ed $N_{DE}$):
		\[
		\sum F_y = 0:\;
		-N_{AD}\,\frac{1}{\sqrt5} - N_{DB}\,\frac{1}{\sqrt2} = 0
		\;\Rightarrow\;
		N_{DB} = \frac{2\sqrt2}{3}F
		\quad\text{(trazione)},
		\]
		\[
		\sum F_x = 0:\;
		-N_{AD}\,\frac{2}{\sqrt5} + N_{DB}\,\frac{1}{\sqrt2}
		+ N_{DE} + 2F = 0
		\;\Rightarrow\;
		N_{DE} = -4F
		\quad\text{(compressione)}.
		...
		\smallskip
		\textit{Nodo $E$} (note $N_{DE}$, $N_{EC}$ e il
		carico $3F$; incognita $N_{BE}$):
		\[
		\sum F_y = 0:\;
		-N_{BE}\,\frac{1}{\sqrt2} - N_{EC}\,\frac{1}{\sqrt5}
		- 3F = 0
		\;\Rightarrow\;
		N_{BE} = -\frac{2\sqrt2}{3}F
		\quad\text{(compressione)};
		\]
		l'equilibrio orizzontale dello stesso nodo,
		$-N_{DE} - N_{BE}/\sqrt2 + 2N_{EC}/\sqrt5 = 0$, è
		identicamente soddisfatto e funge da prima verifica.
		
		\smallskip
		\textit{Nodo $B$} (tutte le aste già note): le due
		equazioni di equilibrio
	\[
	\begin{aligned}
		\sum F_x = 0:\;\;
		&-N_{AB} + N_{BC}
		- N_{DB}\,\frac{1}{\sqrt2} + N_{BE}\,\frac{1}{\sqrt2}
		= 0,\\[2pt]
		\sum F_y = 0:\;\;
		&N_{DB}\,\frac{1}{\sqrt2} + N_{BE}\,\frac{1}{\sqrt2} = 0,
	\end{aligned}
	\]
		sono entrambe verificate, a conferma della coerenza
		della soluzione. $\quad\checkmark$
		...
		\noindent\textbf{Controprova con una sezione di
			Ritter.}
		La cascata sui nodi si è chiusa senza incontrare
		ostacoli: ogni nodo, al momento di affrontarlo,
		presentava al più due aste incognite. Una sezione
		di Ritter non è dunque qui una necessità, ma offre
		una controprova indipendente del pannello centrale
		e l'occasione di rileggerlo con l'analogia della
		trave equivalente. Si taglia la travatura con la
		sezione di \FIGref{fig:tre_maglie_soluzione}\,$(b)$,
		che attraversa $AB$, $DB$ ed $DE$ e isola il
		troncone sinistro (nodi $A$ e $D$). Su di esso
		agiscono le reazioni $X_A = -2F$, $Y_A = \tfrac23 F$
		in $A$ e il carico $2F$ in $D$. I correnti tagliati
		$AB$ ed $DE$ sono orizzontali, dunque \emph{paralleli}:
		i loro poli si individuano per ispezione e cadono su
		nodi della travatura.
		
		\smallskip
		\textit{Corrente superiore $N_{DE}$
			(momento rispetto a $P_{DE}\equiv B$).}
		Rispetto a $B=(\ell,0)$ il carico $2F$ in $D$ ha
		braccio $\ell/3$ (la sua quota) e la reazione $Y_A$
		ha braccio $\ell$; entrambi danno momento orario.
		Lo sforzo $N_{DE}$, orizzontale a quota $\ell/3$, ha
		braccio $\ell/3$:
		\[
		\sum \mu_B = 0:\quad
		-\,\frac{2}{3}F\cdot\ell - 2F\cdot\frac{\ell}{3}
		- N_{DE}\cdot\frac{\ell}{3} = 0
		\;\Rightarrow\;
		N_{DE} = -4F. \quad\checkmark
		\]
		
		\smallskip
		\textit{Corrente inferiore $N_{AB}$
			(momento rispetto a $P_{AB}\equiv D$).}
		Il carico $2F$ è applicato in $D$ e non dà momento
		rispetto al proprio punto di applicazione; resta la
		reazione in $A$. Lo sforzo $N_{AB}$, orizzontale a
		quota nulla, ha braccio $\ell/3$ rispetto a
		$D=(2\ell/3,\ell/3)$:
		\[
		\sum \mu_D = 0:\quad
		-\frac{2}{3}F\cdot\frac{2\ell}{3}
		- \frac{2}{3}F\cdot\frac{\ell}{3}
		+ N_{AB}\cdot\frac{\ell}{3} = 0
		\;\Rightarrow\;
		N_{AB} = \frac{10}{3}F. \quad\checkmark
		\]
		I due contributi della reazione in $A$ provengono
		dalla componente verticale $Y_A$ (braccio $2\ell/3$)
		e dalla componente orizzontale $X_A$ (braccio
		$\ell/3$), entrambi orari.
		
		\smallskip
		\textit{Diagonale $N_{DB}$ (equilibrio verticale).}
		Essendo $AB$ ed $DE$ paralleli, il loro polo comune
		--- polo della diagonale --- è all'infinito e
		l'equazione di momento degenera nell'equilibrio alla
		traslazione verticale del troncone. Le uniche forze
		verticali sono $Y_A$ e la componente verticale di
		$N_{DB}$ (la diagonale è a $45^\circ$):
		\[
		\sum F_y = 0:\quad
		\frac{2}{3}F - N_{DB}\cdot\frac{1}{\sqrt2} = 0
		\;\Rightarrow\;
		N_{DB} = \frac{2\sqrt2}{3}F. \quad\checkmark
		\]
		I tre valori coincidono con quelli ottenuti per
		nodi: la lettura a correnti paralleli (correnti dai
		momenti, diagonale dal taglio) conferma il risultato.
	...
	$N_{AB} = \dfrac{10}{3}F$,\;
	$N_{BC} = \dfrac{14}{3}F$,\;
	$N_{DB} = \dfrac{2\sqrt2}{3}F$ (trazione);\;
	$N_{AD} = -\dfrac{2\sqrt5}{3}F$,\;
	$N_{EC} = -\dfrac{7\sqrt5}{3}F$,\;
	$N_{DE} = -4F$,\;
	$N_{BE} = -\dfrac{2\sqrt2}{3}F$ (compressione).
	
\end{esercizio}

\begin{esercizio}[Mensola reticolare a tre maglie triangolari]
	\label{ex:mensola}
	
Si consideri la travatura reticolare piana di
\FIGref{fig:mensola_schema}, costituita da tre maglie
triangolari. Riferiti a un sistema cartesiano con
origine nel nodo $A$, asse $x$ orizzontale verso destra
e asse $y$ verticale verso l'alto, i nodi hanno
coordinate $A=(0,\,0)$,
$B=(0,\,\ell)$, $C=(\ell/2,\,\ell)$, $D=(\ell,\,\ell)$,
$E=(\ell/2,\,\ell/2)$. Le sette aste sono il montante
$AB$, il corrente superiore in due tratti $BC$ ed $CD$,
e i quattro raggi $AE$, $BE$, $CE$, $DE$ che convergono
nel nodo interno $E$; esse formano le tre maglie
triangolari $ABE$, $BCE$, $CDE$. In $A$ è posto un
carrello con piano di scorrimento verticale (reazione
orizzontale), in $B$ una cerniera. Nei nodi $C$ ed $D$
agiscono due forze verticali dirette verso il basso,
entrambe di intensità $F$. Si determinino gli sforzi
normali in tutte le aste.
	...
	\begin{svolgimento}
		...
		\begin{figure}[htbp]
			...
			\resizebox{0.52\textwidth}{!}{%
				\begin{tikzpicture}[
					...
					\useasboundingbox (-2.2,-1.4) rectangle (5.0,5.9);
					...
					%% carrello in A: piano verticale (reazione orizzontale)
					...
					\node[above,font=\normalsize]at($(C)+(0,1.25)$){$F$};
					\draw[forza]($(D)+(0,1.25)$)--($(D)+(0,0.18)$);
					\node[above,font=\normalsize]at($(D)+(0,1.25)$){$F$};
					...
					%% quota verticale sinistra
					\draw[richiam](A)--++(-1.7,0);
					\draw[richiam](E)--++(-3.7,0);
					\draw[richiam](B)--++(-1.7,0);
					\draw[quota](-1.5,0)--node[left,font=\small]{$\ell/2$}(-1.5,2);
					\draw[quota](-1.5,2)--node[left,font=\small]{$\ell/2$}(-1.5,4);
					
					%% quota orizzontale in basso
					\draw[richiam](A)--++(0,-1.0);
					\draw[richiam](E)--++(0,-3.0);
					\draw[richiam](D)--++(0,-5.0);
					\draw[quota](0,-0.9)--node[below,font=\small]{$\ell/2$}(2,-0.9);
					\draw[quota](2,-0.9)--node[below,font=\small]{$\ell/2$}(4,-0.9);
					
				\end{tikzpicture}
			}
			\caption{Mensola reticolare a tre maglie
				triangolari $ABE$, $BCE$, $CDE$, vincolata a
				una parete verticale. Carrello a piano di
				scorrimento verticale in $A$ (reazione
				orizzontale) e cerniera in $B$; carichi
				verticali $F$ nei nodi $C$ ed $D$.}
			\label{fig:mensola_schema}
		\end{figure}
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		
		\noindent\textbf{Classificazione.}
		Con $n_n = 5$, $n_a = 7$ e $m_\text{est} = 3$
		(carrello in $A$: una componente; cerniera in $B$:
		due componenti):
		\[
		\nu = 2\times 5 - 7 - 3 = 0.
		\]
		...
		\noindent\textbf{Inquadramento meccanico.}
		Lo schema è una \emph{mensola reticolare} ancorata a
		una parete verticale: non potendo nessuno dei due
		vincoli fornire una coppia reattiva propria, è la
		coppia delle due reazioni orizzontali --- quella del
		carrello in $A$ e la componente orizzontale della
		cerniera in $B$ --- a surrogare il momento d'incastro,
		mentre la sola componente verticale disponibile,
		in $B$, equilibra l'intero carico applicato.
		
		\noindent\textbf{Reazioni vincolari.}
		Le incognite esterne sono la reazione orizzontale
		$H_A$ del carrello e le due componenti $H_B$, $V_B$
		della cerniera. Dall'equilibrio globale:
		\[
		\sum F_y = 0:\quad
		V_B - 2F = 0
		\;\Rightarrow\;
		...
		H_B = -\frac{3}{2}F.
		\]
		Il braccio di $H_A$ rispetto a $B$ è la distanza
		verticale $\ell$ fra i due vincoli; i carichi hanno
		bracci orizzontali $\ell/2$ (in $C$) ed $\ell$ (in
		$D$). Le due reazioni orizzontali $H_A=\tfrac32 F$ e
		$H_B=-\tfrac32 F$ formano effettivamente una coppia.
		
		\noindent\textbf{Ispezione: il nodo $C$.}
		I criteri di riconoscimento in senso stretto non
		individuano aste scariche: i nodi a tre o più aste
		($B$, $C$, $E$) o sono caricati o sono vincolati.
		Il nodo $C$ merita però un'osservazione. Vi
		convergono i due correnti allineati $BC$ ed $CD$
		(orizzontali) e il raggio trasversale $CE$
		(verticale); il carico $F$ è anch'esso verticale,
		cioè \emph{diretto lungo} $CE$. L'equilibrio nelle
		due direzioni dà allora immediatamente
		\[
		\sum F_x = 0:\;\; N_{BC} = N_{CD},
		\qquad
		\sum F_y = 0:\;\; N_{CE} = -F .
		\]
		È la versione generalizzata del secondo criterio:
		quando il carico nel nodo è ortogonale ai due
		correnti allineati, questi continuano a trasmettere
		lo stesso sforzo e il raggio trasversale si fa
		interamente carico della forza.
		
		\noindent\textbf{Metodo dei nodi: cascata sui nodi
			canonici.}
		I raggi $AE$ e $DE$ sono inclinati a $45^\circ$, il
		raggio $BE$ a $45^\circ$ verso il basso, $CE$ è
		verticale, $AB$ verticale, $BC$ ed $CD$ orizzontali.
		I nodi $A$ e $D$ presentano due sole aste incognite
		e sono canonici.
		
		\smallskip
		\textit{Nodo $D$} (aste $CD$ orizzontale e $DE$;
		carico $F$):
		\[
		\sum F_y = 0:\;
		-N_{DE}\,\frac{1}{\sqrt2} - F = 0
		\;\Rightarrow\;
		N_{DE} = -\sqrt2\,F
		\quad\text{(compressione)},
		\]
		\[
		\sum F_x = 0:\;
		-N_{CD} - N_{DE}\,\frac{1}{\sqrt2} = 0
		\;\Rightarrow\;
		N_{CD} = F
		\quad\text{(trazione)}.
		\]
		Dall'osservazione al nodo $C$ segue allora
		$N_{BC} = N_{CD} = F$ (trazione) e $N_{CE} = -F$
		(compressione).
		
		\smallskip
		\textit{Nodo $A$} (aste $AB$ verticale e $AE$;
		reazione $H_A$):
		\[
		\sum F_x = 0:\;
		N_{AE}\,\frac{1}{\sqrt2} + \frac{3}{2}F = 0
		\;\Rightarrow\;
		N_{AE} = -\frac{3\sqrt2}{2}F
		\quad\text{(compressione)},
		\]
		\[
		\sum F_y = 0:\;
		N_{AB} + N_{AE}\,\frac{1}{\sqrt2} = 0
		\;\Rightarrow\;
		N_{AB} = \frac{3}{2}F
		\quad\text{(trazione)}.
		\]
		...
		\smallskip
		\textit{Nodo $E$} (note $N_{AE}$, $N_{CE}$, $N_{DE}$;
		incognita $N_{BE}$):
		\[
		\sum F_x = 0:\;
		-N_{AE}\,\frac{1}{\sqrt2} - N_{BE}\,\frac{1}{\sqrt2}
		+ N_{DE}\,\frac{1}{\sqrt2} = 0
		\;\Rightarrow\;
		N_{BE} = \frac{\sqrt2}{2}F
		\quad\text{(trazione)};
		\]
		l'equilibrio verticale dello stesso nodo è
		identicamente soddisfatto e funge da prima verifica.
		...
		\begin{aligned}
			\sum F_x = 0:\;\;
			&N_{BC} + N_{BE}\,\frac{1}{\sqrt2} + H_B = 0,\\[2pt]
			\sum F_y = 0:\;\;
			&-N_{AB} - N_{BE}\,\frac{1}{\sqrt2} + V_B = 0,
		\end{aligned}
		\]
		sono entrambe verificate. $\quad\checkmark$
		...
		\noindent\textbf{Controprova con una sezione di
			Ritter.}
		Si taglia la travatura con la sezione di
		\FIGref{fig:mensola_soluzione}\,$(b)$, che attraversa
		$BC$, $CE$ ed $DE$ e isola la punta caricata
		$\{C,D\}$, soggetta alle sole due forze $F$ in $C$ ed
		in $D$. I tre poli si individuano per ispezione e
		cadono su nodi della travatura.
		
		\smallskip
		\textit{$N_{BC}$ (momento rispetto a
			$P_{BC}\equiv E$).} Il polo è l'intersezione delle
		rette di $CE$ e $DE$, cioè $E$. Il carico in $C$ è
		verticale e passa per $E$ (la retta di $CE$): non dà
		momento. Resta il carico in $D$, con braccio $\ell/2$,
		e lo sforzo $N_{BC}$, orizzontale, con braccio $\ell/2$:
		\[
		\sum \mu_E = 0:\quad
		-F\,\frac{\ell}{2} + N_{BC}\,\frac{\ell}{2} = 0
		\;\Rightarrow\;
		N_{BC} = F. \quad\checkmark
		\]
		
		\smallskip
		\textit{$N_{CE}$ (momento rispetto a
			$P_{CE}\equiv D$).} Il carico in $D$ passa per il
		polo. Resta il carico in $C$ (braccio $\ell/2$) e lo
		sforzo $N_{CE}$, verticale (braccio $\ell/2$):
		\[
		\sum \mu_D = 0:\quad
		F\,\frac{\ell}{2} + N_{CE}\,\frac{\ell}{2} = 0
		\;\Rightarrow\;
		N_{CE} = -F. \quad\checkmark
		\]
		
		\smallskip
		\textit{$N_{DE}$ (momento rispetto a
			$P_{DE}\equiv C$).} Il carico in $C$ passa per il
		polo. Resta il carico in $D$ (braccio $\ell/2$) e lo
		sforzo $N_{DE}$, inclinato a $45^\circ$, con braccio
		$\ell/(2\sqrt2)$ rispetto a $C$:
		\[
		\sum \mu_C = 0:\quad
		-F\,\frac{\ell}{2} - N_{DE}\,\frac{\ell}{2\sqrt2} = 0
		\;\Rightarrow\;
		N_{DE} = -\sqrt2\,F. \quad\checkmark
		\]
		I tre valori coincidono con quelli ottenuti per nodi.
	...
	$N_{AB} = \dfrac{3}{2}F$,\;
	$N_{BC} = N_{CD} = F$,\;
	$N_{BE} = \dfrac{\sqrt2}{2}F$ (trazione);\;
	$N_{AE} = -\dfrac{3\sqrt2}{2}F$,\;
	$N_{CE} = -F$,\;
	$N_{DE} = -\sqrt2\,F$ (compressione).
	
\end{esercizio}

\begin{esercizio}[Travatura a $K$ con carichi all'intradosso]
	\label{ex:ktruss}
	
	Si consideri la travatura reticolare a $K$ di
	\FIGref{fig:ktruss_schema}, formata da quattro celle
	quadrate di lato $d$. I correnti d'intradosso
	($I_0,\dots,I_4$) e d'estradosso ($E_0,\dots,E_4$) sono
	paralleli e distano $d$; ogni montante interno è diviso
	a metà altezza dal nodo di mezzeria $M_i$
	($i=1,\dots,4$), dove convergono i due bracci della $K$.
	La nomenclatura segue la posizione del nodo: $E$
	estradosso, $I$ intradosso, $M$ mezzeria. In $I_0$ è
	posta una cerniera, in $I_4$
	un carrello a reazione verticale; ai nodi d'intradosso
	interni $I_1$, $I_2$, $I_3$ agiscono tre forze
	verticali verso il basso, ciascuna di intensità $F$.
	Si determinino gli sforzi normali in tutte le aste.
	...
	\begin{svolgimento}
		...
		\begin{figure}[htbp]
			...
			\resizebox{0.86\textwidth}{!}{%
				\begin{tikzpicture}[
					scale=0.72,
					asta/.style={line width=1.3pt, line cap=round, line join=round, black},
					nodo/.style={circle, fill=white, draw=black, inner sep=0pt, minimum size=5pt, line width=1.0pt},
					quota/.style={thin, gray, {Stealth[length=4pt,width=3pt]}-{Stealth[length=4pt,width=3pt]}},
					tacca/.style={thin, gray, line cap=round},
					forza/.style={->, >=stealth, line width=1.4pt, black},
					lab/.style={font=\small},
					]
					...
					\useasboundingbox (-1.5,-2.6) rectangle ({4*\dd+1.5},{\hh+1.9});
					
					\draw[asta](E0)--(E1)--(E2)--(E3)--(E4);
					\draw[asta](I0)--(I1)--(I2)--(I3)--(I4);
					\draw[asta](E0)--(I0);
					\foreach \i in {1,2,3,4}{\pgfmathtruncatemacro{\p}{\i-1}
						\draw[asta](E\i)--(M\i); \draw[asta](M\i)--(I\i);
						\draw[asta](M\i)--(E\p); \draw[asta](M\i)--(I\p);}
					
					%% cerniera in I0
					\draw[line width=1.1pt](I0)--++(-0.35,-0.55)--++(0.7,0)--cycle;
					\draw[line width=1.1pt]($(I0)+(-0.48,-0.55)$)--($(I0)+(0.48,-0.55)$);
					\foreach \x in {-0.40,-0.22,-0.04,0.14,0.32}\draw[thin]($(I0)+(\x,-0.55)$)--++(-0.16,-0.20);
					%% carrello in I4
					\draw[line width=1.1pt](I4)--++(-0.35,-0.45)--++(0.7,0)--cycle;
					\draw[line width=1.1pt]($(I4)+(-0.45,-0.45)$)--($(I4)+(0.45,-0.45)$);
					\draw[line width=1.1pt]($(I4)+(-0.22,-0.58)$)circle(3pt);
					\draw[line width=1.1pt]($(I4)+( 0.22,-0.58)$)circle(3pt);
					\draw[line width=1.1pt]($(I4)+(-0.48,-0.73)$)--($(I4)+(0.48,-0.73)$);
					\foreach \x in {-0.40,-0.22,-0.04,0.14,0.32}\draw[thin]($(I4)+(\x,-0.73)$)--++(-0.16,-0.20);
					
					%% carichi F
					\foreach \i in {1,2,3}{ \draw[forza](I\i)--($(I\i)+(0,-1.15)$); \node[below,lab]at($(I\i)+(0,-1.15)$){$F$}; }
					...
					\node[lab,left=2pt]at(I0){$I_0$};
					\node[lab,right=2pt]at(I4){$I_4$};
					\foreach \i in {1,2,3,4}{ \node[lab,right=2pt]at(M\i){$M_\i$}; }
					
					%% quote orizzontali con tacche
					\foreach \i in {0,1,2,3}{ \pgfmathsetmacro{\xa}{\i*\dd}\pgfmathsetmacro{\xb}{(\i+1)*\dd}
						\draw[quota]({\xa},{\hh+0.9})--node[above,font=\small]{$d$}({\xb},{\hh+0.9}); }
					\foreach \i in {0,1,2,3,4}{ \draw[tacca]({\i*\dd},{\hh+0.77})--({\i*\dd},{\hh+1.03}); }
					%% quota verticale h con tacche
					\draw[quota](-0.85,0)--node[left,font=\small]{$d$}(-0.85,\hh);
					\draw[tacca](-0.98,0)--(-0.72,0);
					\draw[tacca](-0.98,\hh)--(-0.72,\hh);
				\end{tikzpicture}
			}
			\caption{Travatura a $K$ a quattro celle
				quadrate di lato $d$; i nodi di mezzeria $M_i$
				stanno a metà altezza dei montanti. Cerniera in
				$I_0$, carrello in $I_4$, carichi $F$ ai nodi
				d'intradosso interni.}
			\label{fig:ktruss_schema}
		\end{figure}
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		
		\noindent\textbf{Classificazione.}
		I nodi sono $n_n = 14$ (cinque d'estradosso, cinque
		d'intradosso, quattro di mezzeria), le aste
		$n_a = 25$ e i vincoli esterni $m_\text{est} = 3$:
		\[
		\nu = 2\times 14 - 25 - 3 = 0.
		\]
		La travatura è isostatica. $\quad\checkmark$
		
		\noindent\textbf{Aste scariche per ispezione.}
		Allo spigolo superiore destro convergono nel solo
		nodo $E_4$ due aste --- il corrente $E_3E_4$ e il
		mezzo montante $E_4M_4$ --- e non vi è carico
		applicato. Per il primo criterio entrambe sono
		scariche:
		\[
		N_{E_3E_4} = 0, \qquad N_{E_4M_4} = 0.
		\]
		È il primo schema del capitolo in cui i criteri di
		riconoscimento danno un esito positivo: la zona di
		estremità presso il carrello, priva di carico, non
		impegna né il corrente superiore né il montante.
		
		\noindent\textbf{Reazioni vincolari.}
		Non vi sono forze orizzontali esterne, dunque la
		componente orizzontale della cerniera è nulla. Per
		il resto, dall'equilibrio globale (carico totale
		$3F$):
		\[
		\sum \mu_{I_0} = 0:\quad
		V_4\,(4d) - F\,d - F\,2d - F\,3d = 0
		\;\Rightarrow\;
		V_4 = \frac{3}{2}F,
		\]
		\[
		...
		V_0 = \frac{3}{2}F.
		\]
		I carichi simmetrici danno reazioni uguali, benché
		la maglia a $K$ non sia simmetrica.
		
		\noindent\textbf{Il nodo della travatura a $K$: la
			sezione taglia quattro aste.}
		A differenza di una travatura a correnti paralleli
		ordinaria, qui una sezione verticale qualsiasi
		interseca \emph{quattro} aste, non tre. Si consideri
		la sezione del pannello centrale in
		\FIGref{fig:ktruss_soluzione}\,$(b)$, che taglia il
		corrente superiore $E_1E_2$, quello inferiore
		$I_1I_2$ e i due bracci $M_2E_1$ ed $M_2I_1$,
		isolando il troncone di sinistra (nodi $I_0$, $E_0$,
		$I_1$, $E_1$, $M_1$). Le tre equazioni cardinali di
		equilibrio del troncone non bastano per quattro
		incognite: il metodo di Ritter, da solo, non chiude
		il problema. La quarta relazione viene
		dall'equilibrio del \emph{mid-nodo} $M_2$.
		
		\smallskip
		\noindent\textit{Equilibrio orizzontale di $M_2$.}
		Nel nodo $M_2$ convergono i due mezzi montanti
		$E_2M_2$ ed $M_2I_2$ --- entrambi verticali, privi
		di componente orizzontale --- e i due bracci
		$M_2E_1$ ed $M_2I_1$, simmetrici rispetto
		all'orizzontale e con identica proiezione
		orizzontale. L'equilibrio lungo $x$ impone allora
		\[
		N_{M_2E_1} + N_{M_2I_1} = 0,
		\]
		cioè i due bracci della $K$ portano sforzi uguali e
		opposti. È la proprietà strutturale caratteristica
		della maglia a $K$.
		
		\smallskip
		\noindent\textit{Diagonali, dall'equilibrio
			verticale del troncone.} I bracci formano con
		l'orizzontale un angolo $\beta$ con
		$\sin\beta = 1/\sqrt5$, $\cos\beta = 2/\sqrt5$
		(salita $d/2$ su corsa $d$). I correnti, orizzontali,
		non hanno componente verticale; restano le due
		reazioni\,/\,carichi e le componenti verticali dei
		bracci:
		\[
		\sum F_y = 0:\quad
		V_0 - F
		+ N_{M_2E_1}\!\left(-\tfrac{1}{\sqrt5}\right)
		+ N_{M_2I_1}\!\left(+\tfrac{1}{\sqrt5}\right) = 0 .
		\]
		Sostituendo $N_{M_2E_1} = -N_{M_2I_1}$ e
		$V_0 = \tfrac32 F$:
		\[
		\frac{F}{2} + \frac{2}{\sqrt5}\,N_{M_2E_1} = 0
		\;\Rightarrow\;
		N_{M_2E_1} = \frac{\sqrt5}{4}F \ \text{(trazione)},
		\quad
		N_{M_2I_1} = -\frac{\sqrt5}{4}F \ \text{(compressione)}.
		\]
		
		\smallskip
		\noindent\textit{Correnti, dall'equilibrio
			orizzontale e dai momenti.} Poiché i due bracci
		hanno sforzi opposti, le loro proiezioni orizzontali
		si elidono; l'equilibrio lungo $x$ del troncone dà
		dunque
		\[
		N_{E_1E_2} + N_{I_1I_2} = 0 .
		\]
		Il momento rispetto a $M_2$ (per cui passano
		entrambi i bracci, che così non compaiono) chiude:
		la reazione $V_0$ ha braccio $2d$, il carico $F$ in
		$I_1$ ha braccio $d$, i due correnti hanno braccio
		$d/2$,
		\[
		\sum \mu_{M_2} = 0:\quad
		-V_0\,(2d) + F\,d
		- N_{E_1E_2}\,\tfrac{d}{2} + N_{I_1I_2}\,\tfrac{d}{2}
		= 0
		\;\Rightarrow\;
		N_{I_1I_2} - N_{E_1E_2} = 4F .
		\]
		Con $N_{E_1E_2} = -N_{I_1I_2}$ si ottiene
		\[
		N_{I_1I_2} = 2F \ \text{(trazione)},
		\qquad
		N_{E_1E_2} = -2F \ \text{(compressione)}.
		\]
		Il pannello è risolto combinando la sezione di
		Ritter (tre equazioni) con l'equilibrio del mid-nodo
		(la quarta): è l'intreccio che la travatura a $K$
		rende necessario.
		...
		\noindent\textbf{Completamento per nodi.}
		Gli altri sforzi si ottengono per nodi, a cascata,
		partendo dai nodi canonici di estremità e usando in
		ciascun montante la relazione dei bracci uguali e
		opposti. Per esempio, al carrello $I_4$ (aste
		$I_3I_4$ orizzontale e $M_4I_4$ verticale, reazione
		$V_4$):
		\[
		\sum F_y = 0:\; N_{M_4I_4} = -\tfrac32 F,
		\qquad
		\sum F_x = 0:\; N_{I_3I_4} = 0;
		\]
		quindi al nodo $M_4$, con $N_{E_4M_4}=0$ e i bracci
		opposti $N_{M_4E_3} = -N_{M_4I_3}$, l'equilibrio
		verticale dà
		$N_{M_4E_3} = -\tfrac{3\sqrt5}{4}F$,
		$N_{M_4I_3} = \tfrac{3\sqrt5}{4}F$. Procedendo
		analogamente nodo per nodo si completa la soluzione,
		riassunta nella \FIGref{fig:ktruss_soluzione}\,$(a)$
		e nel risultato.
		
	\end{svolgimento}
	
	\risultato
	Correnti d'estradosso:
	$N_{E_0E_1}=N_{E_2E_3}=-\tfrac32 F$,\;
	$N_{E_1E_2}=-2F$,\;
	$N_{E_3E_4}=0$.
	Correnti d'intradosso:
	$N_{I_0I_1}=N_{I_2I_3}=\tfrac32 F$,\;
	$N_{I_1I_2}=2F$,\;
	$N_{I_3I_4}=0$.
	Montanti:
	$N_{E_0I_0}=-\tfrac34 F$;\;
	$N_{E_1M_1}=-\tfrac14 F$, $N_{M_1I_1}=\tfrac54 F$;\;
	$N_{E_2M_2}=\tfrac14 F$, $N_{M_2I_2}=\tfrac34 F$;\;
	$N_{E_3M_3}=\tfrac34 F$, $N_{M_3I_3}=\tfrac14 F$;\;
	$N_{E_4M_4}=0$, $N_{M_4I_4}=-\tfrac32 F$.
	Diagonali (bracci di ogni $K$, uguali e opposti):
	$N_{M_1E_0}=-N_{M_1I_0}=\tfrac{3\sqrt5}{4}F$;\;
	$N_{M_2E_1}=-N_{M_2I_1}=\tfrac{\sqrt5}{4}F$;\;
	$N_{M_3E_2}=-N_{M_3I_2}=-\tfrac{\sqrt5}{4}F$;\;
	$N_{M_4E_3}=-N_{M_4I_3}=-\tfrac{3\sqrt5}{4}F$.
	
\end{esercizio}


\begin{esercizio}[Schema reticolare di gru]
	\label{ex:gru}
	
	Si consideri lo schema reticolare piano di gru di
	\FIGref{fig:gru_schema}: un fusto verticale di quattro
	celle quadrate di lato $\ell$, formato dai due correnti
	$A_0\dots A_4$ (posteriore) e $B_0\dots B_4$ (anteriore),
	dai traversi $A_kB_k$ e da una diagonale per cella; e un
	braccio a sbalzo di due moduli, innestato in sommità sul
	corrente anteriore, con estradosso $B_4E_1$, intradosso
	$B_3I_1I_2$, montante $E_1I_1$ e diagonali $B_4I_1$ ed
	$E_1I_2$ (la punta è il triangolo $E_1I_1I_2$). In $A_0$
	è posta una cerniera, in $B_0$ un carrello a reazione
	verticale. I carichi agiscono solo sul braccio: due forze
	verticali verso il basso, ciascuna di intensità $F$, nei
	nodi d'intradosso $I_1$ ed $I_2$. Si determinino gli
	sforzi normali in tutte le aste.
	...
	\begin{svolgimento}
		...
		\begin{figure}[htbp]
			...
			\resizebox{0.52\textwidth}{!}{%
				\begin{tikzpicture}[
					scale=0.85,
					asta/.style={line width=1.3pt, line cap=round, line join=round, black},
					diag/.style={line width=1.0pt, line cap=round, line join=round, black},
					nodo/.style={circle, fill=white, draw=black, inner sep=0pt, minimum size=5pt, line width=1.0pt},
					forza/.style={->, >=stealth, line width=1.4pt, black},
					quota/.style={thin, gray, {Stealth[length=4pt,width=3pt]}-{Stealth[length=4pt,width=3pt]}},
					tacca/.style={thin, gray, line cap=round},
					labM/.style={font=\small},
					labJ/.style={font=\small},
					]
					\def\l{2.0}
					\foreach \k in {0,1,2,3,4}{ \coordinate (A\k) at (0,{\k*\l}); \coordinate (B\k) at (\l,{\k*\l}); }
					\coordinate (E1) at ({2*\l},{4*\l});
					\coordinate (I1) at ({2*\l},{3*\l}); \coordinate (I2) at ({3*\l},{3*\l});
					
					\draw[asta](A0)--(A1)--(A2)--(A3)--(A4);
					\draw[asta](B0)--(B1)--(B2)--(B3)--(B4);
					\foreach \k in {0,1,2,3,4}{ \draw[asta](A\k)--(B\k); }
					\draw[diag](A0)--(B1); \draw[diag](A1)--(B2); \draw[diag](A2)--(B3); \draw[diag](A3)--(B4);
					\draw[asta](B4)--(E1); \draw[asta](B3)--(I1)--(I2); \draw[asta](E1)--(I1);
					\draw[diag](B4)--(I1); \draw[diag](E1)--(I2);
					
					% vincoli
					\draw[line width=1.1pt](A0)--++(-0.32,-0.5)--++(0.64,0)--cycle;
					\draw[line width=1.1pt]($(A0)+(-0.44,-0.5)$)--($(A0)+(0.44,-0.5)$);
					\foreach \x in {-0.36,-0.18,0,0.18,0.30}\draw[thin]($(A0)+(\x,-0.5)$)--++(-0.14,-0.18);
					\draw[line width=1.1pt](B0)--++(-0.32,-0.42)--++(0.64,0)--cycle;
					\draw[line width=1.1pt]($(B0)+(-0.40,-0.42)$)--($(B0)+(0.40,-0.42)$);
					\draw[line width=1.1pt]($(B0)+(-0.18,-0.54)$)circle(2.6pt);
					\draw[line width=1.1pt]($(B0)+( 0.18,-0.54)$)circle(2.6pt);
					\draw[line width=1.1pt]($(B0)+(-0.44,-0.67)$)--($(B0)+(0.44,-0.67)$);
					\foreach \x in {-0.36,-0.18,0,0.18,0.30}\draw[thin]($(B0)+(\x,-0.67)$)--++(-0.14,-0.18);
					
					% carichi
					\draw[forza](I1)--($(I1)+(0,-1.0)$); \node[below,font=\small]at($(I1)+(0,-1.0)$){$F$};
					\draw[forza](I2)--($(I2)+(0,-1.0)$); \node[below,font=\small]at($(I2)+(0,-1.0)$){$F$};
					
					% quotatura
					\foreach \k in {0,1,2,3}{ \draw[quota](-1.0,{\k*\l})--node[left,font=\small]{$\ell$}(-1.0,{(\k+1)*\l}); }
					\foreach \k in {0,1,2,3,4}{ \draw[tacca](-1.13,{\k*\l})--(-0.87,{\k*\l}); }
					\foreach \i in {0,1,2}{ \draw[quota]({\i*\l},{4*\l+1.0})--node[above,font=\small]{$\ell$}({(\i+1)*\l},{4*\l+1.0}); }
					\foreach \i in {0,1,2,3}{ \draw[tacca]({\i*\l},{4*\l+0.87})--({\i*\l},{4*\l+1.13}); }
					
					\foreach \k in {0,1,2,3,4}{ \node[nodo]at(A\k){}; \node[nodo]at(B\k){}; }
					\node[nodo]at(E1){}; \node[nodo]at(I1){}; \node[nodo]at(I2){};
					
					\foreach \k in {0,1,2,3,4}{ \node[labM,left=3pt]at(A\k){$A_\k$}; }
					\foreach \k in {0,1,2}{ \node[labM,right=3pt]at(B\k){$B_\k$}; }
					\node[labM,above=4pt]at(B4){$B_4$};
					\node[labM,below right=-1pt]at(B3){$B_3$};
					\node[labJ,above=2pt]at(E1){$E_1$};
					\node[labJ,below left=-2pt]at(I1){$I_1$};
					\node[labJ,below right=-1pt]at(I2){$I_2$};
				\end{tikzpicture}
			}
			\caption{Schema reticolare di gru: fusto a quattro
				celle quadrate di lato $\ell$ (correnti $A$
				posteriore, $B$ anteriore) e braccio a sbalzo di
				due moduli innestato in sommità sul corrente
				anteriore. Cerniera in $A_0$, carrello in $B_0$;
				carichi $F$ ai nodi d'intradosso $I_1$ ed $I_2$
				del braccio.}
			\label{fig:gru_schema}
		\end{figure}
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		
		\noindent\textbf{Classificazione.}
		Con $n_n = 13$ nodi, $n_a = 23$ aste e
		$m_\text{est} = 3$ (cerniera in $A_0$: due
		componenti; carrello in $B_0$: una):
		\[
		\nu = 2\times 13 - 23 - 3 = 0 .
		\]
		...
		\noindent\textbf{Inquadramento meccanico.}
		Il braccio trasmette in sommità al fusto una forza
		verticale risultante $2F$ e una coppia (il momento di
		sbalzo). Lungo il fusto non agiscono né carichi né
		forze orizzontali: il taglio è ovunque nullo, sicché
		le diagonali risultano scariche e i due correnti
		portano un'azione assiale \emph{uniforme} --- uno
		teso, uno compresso --- che equilibra la coppia. È il
		funzionamento tipico di una gru reticolare, che ora
		ritroviamo nei conti.
		
		\noindent\textbf{Reazioni vincolari.}
		Non vi sono forze orizzontali esterne, quindi la
		componente orizzontale della cerniera è nulla. Per il
		resto:
		\[
		\sum \mu_{A_0} = 0:\quad
		V_{B_0}\,\ell - F\,2\ell - F\,3\ell = 0
		\;\Rightarrow\;
		V_{B_0} = 5F,
		\]
		\[
		...
		V_{A_0} = -3F .
		\]
		L'appoggio posteriore $A_0$ fa da ancoraggio (reazione
		rivolta verso il basso): è il tirante che impedisce il
		ribaltamento, mentre $B_0$ porta la spinta verticale.
		
		\noindent\textbf{Sforzi nel fusto: una sezione
			orizzontale di Ritter.}
		Si recida un generico interpiano del fusto, al di
		sotto della connessione, con una sezione orizzontale
		(\FIGref{fig:gru_soluzione}\,$b$): essa taglia i due
		correnti $A$, $B$ e la diagonale, isolando un troncone
		superiore soggetto alle sole due forze $F$. Si prenda
		come polo il nodo anteriore $B_{k+1}$, intersezione
		del corrente $B$ e della diagonale, che così non
		compaiono. Il momento dei due carichi rispetto ad esso
		--- forze verticali, contano i soli bracci orizzontali
		$\ell$ e $2\ell$ --- vale
		\[
		\sum \mu_{B_{k+1}} = 0:\quad
		N_A\,\ell - F\,\ell - F\,2\ell = 0
		\;\Rightarrow\;
		N_A = 3F \ \text{(trazione)} .
		\]
		Specularmente, il momento rispetto a un nodo del
		corrente posteriore dà
		$N_B\,\ell = F\,2\ell + F\,3\ell$, cioè
		$N_B = -5F$ (compressione); l'equilibrio verticale del
		troncone, $N_B - N_A = -2F$ già soddisfatto, lascia la
		diagonale scarica.
		
		\smallskip
		Il punto cruciale è che \emph{il momento rispetto al
			polo non dipende dalla quota del taglio}: i bracci
		orizzontali dei carichi sono sempre $\ell$ e $2\ell$,
		quale che sia il livello reciso. Dunque $N_A=3F$ ed
		$N_B=-5F$ sono gli stessi a ogni interpiano: i correnti
		del fusto hanno sforzo uniforme lungo l'altezza, teso
		il posteriore e compresso l'anteriore. Ne discende
		anche che \emph{l'aggiunta di ulteriori piani non
			cambierebbe nulla}: ogni nuovo interpiano
		ripeterebbe identico lo stesso stato --- correnti a
		$\pm$, diagonali e traversi scarichi.
		
		\noindent\textbf{Il braccio, per nodi.}
		Le diagonali del braccio sono a $45^\circ$. Partendo
		dalla punta $I_2$ e risalendo verso il fusto,
		l'equilibrio di ciascun nodo (proiezione lungo le due
		direzioni) fornisce
		\[
		\begin{array}{@{}l@{\qquad}l@{\qquad}l@{}}
			\text{nodo } I_2: & N_{E_1I_2} = \sqrt2\,F, & N_{I_1I_2} = -F;\\[2pt]
			\text{nodo } E_1: & N_{B_4E_1} = F,          & N_{E_1I_1} = -F;\\[2pt]
			\text{nodo } I_1: & N_{B_4I_1} = 2\sqrt2\,F, & N_{B_3I_1} = -3F.
		\end{array}
		\]
		Le aste della prima colonna ($E_1I_2$, $B_4E_1$,
		$B_4I_1$) risultano tese, quelle della seconda
		($I_1I_2$, $E_1I_1$, $B_3I_1$) compresse.
		
		\noindent\textbf{La cella di trasferimento.}
		Resta la cella di sommità, dove la coppia del braccio
		si converte nella coppia dei correnti. Allo spigolo
		$A_4$ convergono due sole aste scariche
		($N_{A_3A_4}=N_{A_4B_4}=0$). Al nodo $B_4$, noti gli
		sforzi del braccio,
		\[
		\sum F_x = 0:\; N_{A_3B_4} = 3\sqrt2\,F,
		\qquad
		\sum F_y = 0:\; N_{B_3B_4} = -5F ;
		\]
		e al nodo $A_3$
		\[
		\sum F_x = 0:\; N_{A_3B_3} = -3F,
		\qquad
		\sum F_y = 0:\; N_{A_2A_3} = 3F .
		\]
		La diagonale $A_3B_4$ e il traverso $A_3B_3$ assorbono
		il trasferimento; da $A_3$ in giù i correnti assumono i
		valori costanti $+3F$ e $-5F$ già ottenuti con la
		sezione di Ritter, e le restanti diagonali e traversi
		del fusto sono scarichi.
	...
	\risultato
	Fusto --- correnti a sforzo uniforme: corrente
	posteriore $N_{A_0A_1}=N_{A_1A_2}=N_{A_2A_3}=3F$
	(trazione), $N_{A_3A_4}=0$; corrente anteriore
	$N_{B_0B_1}=\dots=N_{B_3B_4}=-5F$ (compressione).
	Diagonali e traversi del fusto scarichi, tranne la cella
	di trasferimento: $N_{A_3B_4}=3\sqrt2\,F$,
	$N_{A_3B_3}=-3F$. Braccio: $N_{B_4E_1}=F$,
	$N_{B_4I_1}=2\sqrt2\,F$, $N_{E_1I_2}=\sqrt2\,F$
	(trazione); $N_{B_3I_1}=-3F$, $N_{I_1I_2}=-F$,
	$N_{E_1I_1}=-F$ (compressione). Reazioni:
	$V_{B_0}=5F$, $V_{A_0}=-3F$, $H_{A_0}=0$.
