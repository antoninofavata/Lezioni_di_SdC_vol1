...
%----------------------------------------------------------------------------------------------------------------------------
%  SEZIONE DI CHIUSURA DEL CAPITOLO -- da accodare a
%  C04_Sistemi_connessioni_eterogenee.tex
%  Primo blocco: preambolo + esercizio del ponte (enunciato, schema, classificazione, strategia)
%  I tre casi (a),(b),(c) e il secondo esercizio (progetto) seguiranno.
%----------------------------------------------------------------------------------------------------------------------------
\section{Esercizi svolti}
\label{sec:esercizi_eterogenei}
%----------------------------------------------------------------------------------------------------------------------------

Gli esempi sviluppati nelle sezioni precedenti hanno illustrato la
sequenza operativa su tipologie a gerarchia esplicita, nelle quali un
sottosistema \emph{portato} si risolve per primo e scarica le proprie
azioni sul \emph{portante}. Gli esercizi che seguono mettono alla prova
quel metodo su un sistema che \emph{non} rientra in tale schema: il suo
primo atto analitico è proprio il riconoscimento dell'\emph{assenza} di
gerarchia portante/portato e la conseguente scelta di una risoluzione
globale. Il primo esercizio classifica e risolve il sistema sotto tre
scenari di carico, mostrando come il cammino dei carichi --- e con esso
il segno delle azioni nei singoli membri --- dipenda dal carico e non
solo dalla struttura. Il secondo \emph{inverte} l'impostazione: fissata una prestazione, ne ricava il vincolo geometrico che la realizza --- un modo di procedere che trascende il ponte. Esso poggia sulla sola \emph{isostaticità} --- per cui le azioni interne discendono dall'equilibrio, indipendentemente dalla rigidezza --- e vale per ogni sistema isostatico in cui la prestazione richiesta si lasci esprimere come condizione sulle azioni interne.


%............................................................................................................................
\begin{esercizio}[Ponte a stampella: arco a tre cerniere di anelli a tre cerniere]
	\label{ex:ponte_stampella}
	
	Si consideri il \emph{ponte a stampella} di
	Figura~\ref{fig:stampella_schema}. Due travi rettilinee $CDE$ ed $EFG$,
	allineate alla quota $\ell/4$ sopra gli appoggi, sono sorrette da quattro
	bielle inclinate $AC$, $AD$, $BF$, $BG$ incernierate al suolo nei piedi
	$A$ e $B$. Tutte le connessioni sono cerniere: \emph{esterne} in $A$ e
	$B$, \emph{interne} in $C$, $D$, $E$, $F$, $G$; in particolare $E$ è la
	cerniera di \emph{chiave} fra le due travi, mentre in $D$ ed $F$ le
	bielle si innestano sulle travi continue senza interromperne la
	continuità rotazionale. Le misure sono quotate in Figura~\ref{fig:stampella_schema}: luce fra gli appoggi $\overline{AB}=4\ell$, ribassamento dell'impalcato $\ell/4$, sbalzi orizzontali delle bielle pari a $\ell/2$ (esterne $AC$, $BG$) ed $\ell$ (interne $AD$, $BF$).
	
	La struttura va studiata sotto tre scenari di carico, tutti con carico
	distribuito $p$ verticale discendente:
	\begin{itemize}
		\item[(a)] \,$p$ esteso a tutta la travata $CDEFG$;
		\item[(b)] \,$p$ sul solo tratto centrale $DEF$;
		\item[(c)] \,$p$ su $CD$ ed $EF$ (scenario asimmetrico).
	\end{itemize}
	Per la struttura così definita:
	\begin{itemize}
		\item[1)] se ne dia la classificazione statico-cinematica,
		riconoscendo le eventuali gerarchie strutturali;
		\item[2)] per ciascuno scenario si determinino le reazioni dei
		vincoli esterni e interni e si traccino i \textit{diagrammi quotati}
		della forza normale $N$, del taglio $T$ e del momento flettente $M$.
	\end{itemize}
...
%				\foreach \gy in {0,1,2,3,4} \draw[griglia] (-2,\gy)--(18,\gy);
	...
	\paragraph{1.~Classificazione statico-cinematica.}
	La classificazione è puramente topologica e \emph{indipendente dal
		carico}: vale dunque una volta per tutti e tre gli scenari.
	
	\medskip\noindent\textbf{Composizione.}
	La struttura è formata da \emph{sei corpi rigidi}: le quattro bielle
	$AC$, $AD$, $BF$, $BG$ (aste incernierate ai due estremi) e le due travi
	rettilinee $CDE$ ed $EFG$. Le connessioni sono tutte cerniere: due
	\emph{esterne} ai piedi $A$ e $B$ --- che vincolano al suolo,
	rispettivamente, le coppie di bielle $(AC,AD)$ e $(BF,BG)$ --- e cinque
	\emph{interne}: complete in $C$ ($AC$--$CDE$), $E$ ($CDE$--$EFG$, chiave)
	e $G$ ($EFG$--$BG$); in $D$ ed $F$ la cerniera collega la biella ($AD$,
	$BF$) alla trave continua senza interromperne la continuità.
	
	\medskip\noindent\textbf{Conteggio dei vincoli.}
	Con $n_e=6$ corpi, le molteplicità di vincolo valgono
	\[
	m_{\mathrm{est}} = \underbrace{4}_{A} + \underbrace{4}_{B} = 8,
	\qquad
	m_{\mathrm{int}} = \underbrace{2}_{C} + \underbrace{2}_{E} + \underbrace{2}_{G}
	+ \underbrace{2}_{D} + \underbrace{2}_{F} = 10,
	\]
	poiché ciascuna cerniera a terra vincola \emph{due} bielle al suolo
	($2\times2=4$) e ciascuna cerniera interna connette due corpi. Il
	conteggio di Müller-Breslau dà
	\[
	\nu = 3\,n_e - m_{\mathrm{int}} - m_{\mathrm{est}} = 18 - 10 - 8 = 0:
	\]
	la struttura è (potenzialmente) isostatica.
	
	\medskip\noindent\textbf{Lettura strutturale: arco a tre cerniere di anelli
		a tre cerniere.}
	Ciascun semiarco è un \emph{anello a tre cerniere}: a sinistra il
	triangolo $A$--$C$--$D$, formato dalle bielle $AC$ ed $AD$ e dal tratto
	$CD$ della trave, con cerniere in $A$, $C$, $D$; a destra il triangolo
	$B$--$F$--$G$ (bielle $BF$, $BG$ e tratto $FG$). Un anello chiuso di tre
	corpi con tre cerniere non allineate è internamente rigido e isostatico.
	Ogni anello prosegue poi rigidamente fino alla chiave --- col tratto $DE$
	a sinistra ed $EF$ a destra, appartenenti alle medesime travi --- sicché
	ciascun semiarco è un \emph{unico corpo rigido}, incernierato a terra in
	$A$ (risp.\ $B$) e collegato in $E$. I due semiarchi rigidi, uniti dalla
	cerniera di chiave $E$, costituiscono allora un arco a tre
		cerniere $A$--$E$--$B$ (\PARref{sec:arco_tre_cerniere}): con $A$, $E$,
	$B$ non allineati ($E$ a quota $\ell/4$ sopra $AB$), il complesso è
	globalmente isostatico, in accordo col conteggio $\nu=0$. Le
	bielle convergenti in ciascun piede, del resto, hanno risultante passante
	per il piede e dunque \emph{equivalgono a una cerniera} in $A$,
	risp.\ $B$.
	
	\medskip\noindent\textbf{Gerarchia strutturale.}
	Non vi è gerarchia \emph{portante/portato} a cascata: i due semiarchi
	sono paritetici e collegati alla chiave $E$. La risoluzione è
	dunque \emph{globale}, e sfrutta la condizione $M_E=0$ alla cerniera di
	chiave. Essendo \emph{scariche}, le quattro bielle sono soggette al solo
	\textit{sforzo assiale}: note le reazioni di base, le loro azioni seguono
	per equilibrio dei nodi $A$ e $B$. Si osservi infine che la
	\emph{geometria} è simmetrica rispetto alla verticale per $E$: ne segue
	che per gli scenari di carico simmetrici (a) e (b) anche reazioni e
	caratteristiche saranno simmetriche, mentre lo scenario (c) romperà la
	simmetria.
	
	\paragraph{2.~Strategia risolutiva (comune ai tre scenari).}
	Poiché la classificazione vale per tutti i casi, anche il procedimento è
	unico: cambiano solo i valori numerici. Esso specializza i passi della
	\SECref{sec:sequenza_operativa} alla lettura ad arco a tre cerniere.
	
	\medskip\noindent\textbf{Passo~1 --- Reazioni esterne (metodo delle tre
		cerniere).}
	Le incognite esterne sono le quattro componenti di reazione nelle due
	cerniere a terra $(H_A,V_A)$, $(H_B,V_B)$. Le tre equazioni cardinali
	della statica forniscono $H_A+H_B=0$, $V_A+V_B = \text{(risultante dei
		carichi)}$ e l'equilibrio alla rotazione; la quarta equazione è la
	condizione di \emph{chiave} $M_E=0$, imposta sul semiarco di sinistra
	(momento attorno a $E$ delle sole forze a sinistra di $E$). Quest'ultima
	lega la spinta orizzontale al ``momento da trave appoggiata'' $M_E^{0}$
	attraverso il ribassamento $f=\ell/4$:
	\[
	H_A\cdot f = M_E^{0}\ \Longrightarrow\ H_A=\frac{M_E^{0}}{f}.
	\]
	Il ribassamento modesto ($f=\ell/4$ a fronte di una luce $4\ell$) rende
	la spinta $H_A$ elevata: è la caratteristica saliente di questo sistema.
	
	\medskip\noindent\textbf{Passo~2 --- Reazioni interne (bielle e chiave).}
	Note le reazioni di base, l'equilibrio di ciascun piede ripartisce la
	reazione sulle due bielle convergenti. Assumendo le bielle \emph{tese}
	($N>0$) con versori diretti dal piede verso l'altro estremo,
	\[
	\mathbf a_{AC}=\tfrac{1}{\sqrt5}(-2,\,1),\qquad
	\mathbf a_{AD}=\tfrac{1}{\sqrt{17}}(4,\,1),
	\]
	le pendenze legano le componenti orizzontali a quelle verticali e
	l'equilibrio del nodo $A$ (analogamente $B$) fornisce gli sforzi assiali;
	un segno negativo segnala compressione. Le componenti che le bielle
	trasmettono alle travi in $C$, $D$ (e $F$, $G$) diventano carichi noti.
	Alla chiave $E$ i due semiarchi si scambiano la sola spinta orizzontale
	$H_E$ quando il carico è simmetrico, cui si aggiunge una componente
	verticale $V_E$ nello scenario asimmetrico.
	
	
	\medskip\noindent\textbf{Passi~3--4 --- Caratteristiche e diagrammi.}
	Assemblate le azioni di contatto, si tracciano $N$, $T$, $M$ sulle due
	travi rettilinee, adottando un'ascissa $z$ crescente da $C$ verso $G$ e
	riportando il momento dal lato delle \emph{fibre tese}. Il segno delle
	azioni nelle bielle --- e dunque se la travata risulti tesa o compressa
	--- si legge comodamente al nodo $A$ col \emph{poligono delle forze}: le
	due bielle sono entrambe puntoni finché la reazione $\mathbf r_A$ cade
	\emph{dentro} il ventaglio $A$--$C$--$D$; quando $\mathbf r_A$ ne esce
	(oltre la biella interna $AD$), la biella esterna $AC$ inverte in tirante
	e l'estremità di travata si comprime. Questa lettura, sviluppata nella
	discussione che chiude l'esercizio, sarà il filo conduttore del confronto
	fra i tre scenari.
	
	
	%----------------------------------------------------------------------------------------------------------------------------
	%  CASO (a) -- carico $p$ su tutta la travata $CDEFG$
	%----------------------------------------------------------------------------------------------------------------------------
	
	\medskip\noindent\textbf{Caso (a) --- carico $p$ su tutta la travata $CDEFG$.}
	\nopagebreak
	
	\medskip\noindent\textbf{Reazioni esterne.}
	La risultante del carico vale $p\cdot 5\ell = 5p\ell$. Geometria e carico
	sono simmetrici rispetto alla verticale per la chiave $E$, perciò
	\[
	V_A=V_B=\tfrac{5}{2}\,p\ell\ (\uparrow),\qquad H_A+H_B=0.
	\]
	La condizione di chiave $M_E=0$, imposta sul semiarco di sinistra, lega la
	spinta al momento ``da trave appoggiata''
	$M_E^{0}=V_A(2\ell)-\tfrac{5}{2}p\ell\cdot\tfrac{5}{4}\ell=\tfrac{15}{8}p\ell^2$,
	equilibrato col braccio $f=\ell/4$:
	\[
	H_A=\frac{M_E^{0}}{f}=\tfrac{15}{2}\,p\ell\ (\rightarrow),\qquad
	H_B=\tfrac{15}{2}\,p\ell\ (\leftarrow).
	\]
	La spinta è elevata --- circa tre volte il carico totale di metà travata
	--- proprio per il modesto ribassamento $f=\ell/4$.
	
	\medskip\noindent\textbf{Sforzi nelle bielle.}
	L'equilibrio del nodo $A$ (proiezioni su $x$ e $y$, bielle assunte tese)
	fornisce le componenti verticali $V_{AC}=-\tfrac{5}{12}p\ell$,
	$V_{AD}=-\tfrac{25}{12}p\ell$, da cui $N=V/a_y$:
	\[
	N_{AC}=N_{BG}=-\tfrac{5\sqrt5}{12}\,p\ell,\qquad
	N_{AD}=N_{BF}=-\tfrac{25\sqrt{17}}{12}\,p\ell .
	\]
	I segni negativi dicono che tutte e quattro le bielle sono \emph{compresse}
	(puntoni); le aste d'imposta $AD$, $BF$ --- in modulo circa nove volte le
	$AC$, $BG$ --- incanalano la spinta verso i piedi. Alla chiave i due
	semiarchi si scambiano la sola spinta orizzontale $H_E=\tfrac{15}{2}p\ell$
	($V_E=0$ per simmetria).
	
	\medskip\noindent\textbf{Caratteristiche della sollecitazione.}
	Con ascissa $z$ crescente da $C$ verso $G$ e momento dal lato delle fibre
	tese (valori in $p\ell$ per $N,T$, in $p\ell^2$ per $M$), lo sforzo normale
	è costante a tratti:
	\[
	\begin{array}{l|cccc}
		\text{tratto} & CD & DE & EF & FG\\\hline
		N & +\tfrac{5}{6} & -\tfrac{15}{2} & -\tfrac{15}{2} & +\tfrac{5}{6}
	\end{array}
	\]
	trazione modesta sui tratti d'estremità $CD$, $FG$ e compressione pari alla
	spinta $-\tfrac{15}{2}p\ell$ su tutto il corrente centrale $DE$--$EF$
	(continua attraverso la chiave $E$). Il \emph{taglio} è lineare a tratti e
	\emph{emisimmetrico}: su $CD$ passa da $+\tfrac{5}{12}$ (in $C$) a
	$-\tfrac{13}{12}$ (in $D$), annullandosi in $s=\tfrac{5}{12}\ell$; su $DE$
	va da $+1$ a $0$; specularmente su $EF$ ($0$ a $-1$) e su $FG$
	($+\tfrac{13}{12}$ a $-\tfrac{5}{12}$), con salti in $D$ ed $F$ per le forze
	concentrate trasmesse dalle bielle. Il \emph{momento} è \emph{simmetrico},
	nullo alle tre cerniere $C$, $E$, $G$, parabolico su ciascun tratto (carico
	ripartito) con spigolo nei pendoli $D$, $F$: presenta i colmi positivi
	$+\tfrac{25}{288}p\ell^2$ su $CD$ e $FG$ e i picchi negativi
	$M_D=M_F=-\tfrac12 p\ell^2$. I diagrammi sono riportati in
	Figura~\ref{fig:stampella_NTM_a}.
	...
	\medskip\noindent\textbf{Lettura ad arco.}
	I diagrammi rivelano un netto comportamento ad arco. Il cammino
	$A$--$D$--$E$--$F$--$B$ è interamente \emph{compresso}: la spinta, immessa
	dalle aste d'imposta $AD$, $BF$, attraversa il corrente centrale
	($N_{DE}=N_{EF}=-\tfrac{15}{2}p\ell$, in modulo pari ad $H_E$) e si scarica
	ai piedi. I tratti d'estremità $CD$, $FG$, invece, sono \emph{tesi}
	($+\tfrac56 p\ell$) e fanno da \emph{catena}: chiudono orizzontalmente i due
	anelli d'imposta $A$--$C$--$D$ e $B$--$F$--$G$, assorbendo la componente
	orizzontale che le bielle trasmettono alle travi in $C$ e in $G$. È una
	catena \emph{locale} a ciascun anello, non una catena dell'intera travata.
	
	
	%----------------------------------------------------------------------------------------------------------------------------
	%  CASO (b) -- carico p sul solo tratto centrale DEF
	%----------------------------------------------------------------------------------------------------------------------------
	
	\medskip\noindent\textbf{Caso (b) --- carico $p$ sul solo tratto centrale $DEF$.}
	\nopagebreak
	
	\medskip\noindent\textbf{Reazioni esterne.}
	Risultante del carico $p\cdot 2\ell=2p\ell$; per simmetria
	$V_A=V_B=p\ell\ (\uparrow)$, $H_A+H_B=0$. La condizione di chiave $M_E=0$ dà
	$M_E^{0}=V_A(2\ell)-p\ell\cdot\tfrac{\ell}{2}=\tfrac32 p\ell^2$ (concorre solo
	la metà sinistra $DE$ del carico, baricentro a $\ell/2$ da $E$), onde
	\[
	H_A=\frac{M_E^{0}}{f}=6\,p\ell\ (\rightarrow),\qquad H_B=6\,p\ell\ (\leftarrow).
	\]
	
	\medskip\noindent\textbf{Sforzi nelle bielle.}
	L'equilibrio del nodo $A$ fornisce $V_{AC}=+\tfrac13 p\ell$,
	$V_{AD}=-\tfrac43 p\ell$, da cui
	\[
	N_{AC}=N_{BG}=+\tfrac{\sqrt5}{3}\,p\ell\ (\text{traz.}),\qquad
	N_{AD}=N_{BF}=-\tfrac{4\sqrt{17}}{3}\,p\ell\ (\text{compr.}).
	\]
	Rispetto al caso (a) le bielle d'estremità $AC$, $BG$ hanno
	\emph{cambiato segno}: ora sono \emph{tiranti}. Alla chiave $H_E=6\,p\ell$,
	$V_E=0$.
	
	\medskip\noindent\textbf{Caratteristiche della sollecitazione.}
	Con le stesse convenzioni del caso (a) (valori in $p\ell$ per $N,T$, in
	$p\ell^2$ per $M$):
	\[
	\begin{array}{l|cccc}
		\text{tratto} & CD & DE & EF & FG\\\hline
	...
	\end{array}
	\]
	\emph{tutti i tratti sono compressi}: il corrente centrale porta
	$-6p\ell=-H_E$, i tratti d'estremità una compressione modesta
	$-\tfrac23 p\ell$. Il \emph{taglio} (emisimmetrico, nullo in $E$) è uniforme
	$-\tfrac13$ su $CD$, va da $+1$ a $0$ su $DE$, da $0$ a $-1$ su $EF$ ed è
	uniforme $+\tfrac13$ su $FG$. Il \emph{momento} è nullo alle tre cerniere,
	lineare su $CD$, $FG$ (scarichi) e parabolico su $DE$, $EF$ (caricati), con
	picchi $M_D=M_F=-\tfrac12 p\ell^2$ e mezzeria $-\tfrac18 p\ell^2$; è
	\emph{ovunque non positivo} (fibre tese all'estradosso), a differenza del
	caso (a). I diagrammi sono in Figura~\ref{fig:stampella_NTM_b}.
	...
	Concentrando il carico sul tratto centrale, l'impalcato risulta
	\emph{interamente compresso} e le bielle d'estremità si invertono in
	tiranti: è la commutazione rispetto al caso (a), su cui si tornerà nella
	sintesi.

	%----------------------------------------------------------------------------------------------------------------------------
	%  CASO (c) -- carico asimmetrico (p su CD ed EF)
	%----------------------------------------------------------------------------------------------------------------------------
	
	\medskip\noindent\textbf{Caso (c) --- carico asimmetrico ($p$ su $CD$ ed $EF$).}
	\nopagebreak
	
	\medskip\noindent\textbf{Reazioni esterne.}
	Le risultanti dei distribuiti, dirette verso il basso, valgono
	$P_{CD}=\tfrac32 p\ell$ (baricentro $x=\tfrac{\ell}{4}$) e $P_{EF}=p\ell$
	(baricentro $x=\tfrac52\ell$). L'equilibrio globale fornisce $H_A+H_B=0$,
	$V_A+V_B=\tfrac52 p\ell$ e
	\[
	\textstyle\sum M_A=0:\quad 4\ell\,V_B=P_{CD}\,\tfrac{\ell}{4}+P_{EF}\,\tfrac52\ell
	\ \Rightarrow\ V_B=\tfrac{23}{32}p\ell,\quad V_A=\tfrac{57}{32}p\ell.
	\]
	La condizione di chiave, imposta sul semiarco di sinistra, fornisce la spinta
	\[
	H_A=\tfrac{15}{4}p\ell\ (\rightarrow),\qquad H_B=\tfrac{15}{4}p\ell\ (\leftarrow).
	\]
	
	\medskip\noindent\textbf{Sforzi nelle bielle.}
	L'equilibrio dei nodi $A$ e $B$ dà
	\[
	N_{AC}=-\tfrac{9\sqrt5}{16}p\ell,\quad N_{AD}=-\tfrac{39\sqrt{17}}{32}p\ell,\quad
	N_{BF}=-\tfrac{83\sqrt{17}}{96}p\ell,\quad N_{BG}=+\tfrac{7\sqrt5}{48}p\ell:
	\]
	tre bielle compresse e la sola $BG$ tesa --- effetto dell'asimmetria. Caduta
	la simmetria, alla chiave i due semiarchi si scambiano, oltre alla spinta
	$H_E=\tfrac{15}{4}p\ell$, anche una componente verticale
	$V_E=\tfrac{9}{32}p\ell\neq0$.
	...
	\begin{array}{l|cccc}
		\text{tratto} & CD & DE & EF & FG\\\hline
		N & +\tfrac98 & -\tfrac{15}{4} & -\tfrac{15}{4} & -\tfrac{7}{24}
	\end{array}
	\]
	trazione su $CD$, compressione pari alla spinta $-\tfrac{15}{4}p\ell$ sul
	corrente centrale e lieve compressione su $FG$. Il \emph{taglio} è lineare
	sotto i carichi: su $CD$ passa da $+\tfrac{9}{16}$ a $-\tfrac{15}{16}$ (zero
	in $s=\ell/16$), vale $+\tfrac{9}{32}$ costante su $DE$, va da
	$+\tfrac{9}{32}$ a $-\tfrac{23}{32}$ su $EF$ e vale $+\tfrac{7}{48}$ costante
	su $FG$. Il \emph{momento} si annulla alle tre cerniere, è parabolico sui
	tratti caricati e lineare altrove, con massimi locali $+\tfrac{81}{512}p\ell^2$
	(su $CD$) e $+\tfrac{81}{2048}p\ell^2$ (su $EF$) e i valori
	$M_D=-\tfrac{9}{32}p\ell^2$, $M_F=-\tfrac{7}{32}p\ell^2$ ai pendoli. I
	diagrammi sono in Figura~\ref{fig:stampella_NTM_c}.
	...
	\begin{figure}[htbp!]\centering
		\resizebox{\linewidth}{!}{%
			\begin{tikzpicture}[x=1.5cm,y=1.5cm,font=\scriptsize,
				...
				\def\sM{1.3}\def\sT{0.42}\def\sN{0.10}\def\sNc{0.14}\def\sNf{0.30}
				\newcommand{\nodi}{%
					\coordinate (C) at (-0.5,0);\coordinate (D) at (1,0);\coordinate (E) at (2,0);
					\coordinate (F) at (3,0);\coordinate (G) at (4.5,0);
					\coordinate (A) at (0,-0.25);\coordinate (B) at (4,-0.25);}
				...
				\begin{scope}[shift={(0,5.2)}]
					...
					\draw[biella] (A)--(C);\draw[biella] (A)--(D);\draw[biella] (B)--(F);\draw[biella] (B)--(G);
					\draw[trave] (C)--(G);
					\draw[bordo] (-0.5,0.225)--(1,0.225);
					\foreach \x in {-0.5,-0.25,...,1.001}\draw[carico] (\x,0.225)--(\x,0);
					\node[above] at (0.25,0.265){$p$};
					\draw[bordo] (2,0.225)--(3,0.225);
					\foreach \x in {2,2.25,...,3.001}\draw[carico] (\x,0.225)--(\x,0);
					\node[above] at (2.5,0.265){$p$};
					\gnd{A}\gnd{B}
					\draw[reaz] (A)--($(A)+(0.55,0)$);\node[gray!70,below,font=\tiny]at($(A)+(0.33,-0.04)$){$\tfrac{15}{4}p\ell$};
					\draw[reaz] ($(A)+(0,-0.80)$)--($(A)+(0,-0.27)$);\node[gray!70,left,font=\tiny]at($(A)+(0.02,-0.62)$){$\tfrac{57}{32}p\ell$};
					\draw[reaz] (B)--($(B)+(-0.55,0)$);\node[gray!70,below,font=\tiny]at($(B)+(-0.33,-0.04)$){$\tfrac{15}{4}p\ell$};
					\draw[reaz] ($(B)+(0,-0.80)$)--($(B)+(0,-0.27)$);\node[gray!70,right,font=\tiny]at($(B)+(-0.02,-0.62)$){$\tfrac{23}{32}p\ell$};
					\node[nodoV] at (C){};\node[nodoV] at (E){};\node[nodoV] at (G){};
					\node[nodoV] at (A){};\node[nodoV] at (B){};
					\semic{D}\semic{F}
					...
					\node[font=\tiny] at (2,-1.05){(a) schema, carichi e reazioni};
				\end{scope}
				
				%==================== (b) NORMALE N ====================
				\begin{scope}[shift={(0,3.4)}]
					\nodi
					\diagLin{C}{D}{0}{1}{1.125}{1.125}{\sNc}
					\diagLin{D}{E}{0}{1}{-3.75}{-3.75}{\sN}
					\diagLin{E}{F}{0}{1}{-3.75}{-3.75}{\sN}
					\diagLin{F}{G}{0}{1}{-0.29167}{-0.29167}{\sNf}
					\assi
					\node[segno] at ($(C)!0.5!(D)+(0,0.55*1.125*\sNc)$){$+$};
					\node[segno] at ($(D)!0.5!(F)+(0,-0.5*3.75*\sN)$){$-$};
					\node[segno] at ($(F)!0.5!(G)+(0,-0.55*0.29167*\sNf)$){$-$};
					\node[above,font=\tiny] at ($(C)!0.5!(D)+(0,1.125*\sNc)$){$\tfrac{9}{8}p\ell$};
					\node[below,font=\tiny] at ($(D)!0.5!(E)+(0,-3.75*\sN)$){$\tfrac{15}{4}p\ell$};
					\node[below,font=\tiny] at ($(F)!0.5!(G)+(0,-0.29167*\sNf)$){$\tfrac{7}{24}p\ell$};
					\node[font=\tiny] at (2,-0.92){(b) sforzo normale $N$\ \ {\tiny($CD$,\,$FG$: ordinate amplificate)}};
				\end{scope}
				...
				\begin{scope}[shift={(0,1.7)}]
					\nodi
					\diagLin{C}{D}{0}{1}{0.5625}{-0.9375}{\sT}
					\diagLin{D}{E}{0}{1}{0.28125}{0.28125}{\sT}
					\diagLin{E}{F}{0}{1}{0.28125}{-0.71875}{\sT}
					\diagLin{F}{G}{0}{1}{0.14583}{0.14583}{\sT}
					\assi
					\node[segno] at ($(C)+(0.10,0.08)$){$+$};
					\node[segno] at ($(C)!0.90!(D)+(0,-0.42*\sT)$){$-$};
					\node[segno] at ($(D)!0.5!(E)+(0,0.5*0.28125*\sT)$){$+$};
					%\node[segno] at ($(E)+(0.06,0.06)$){$+$};
					\node[segno] at ($(E)!0.84!(F)+(0,-0.30*\sT)$){$-$};
					\node[segno] at ($(F)!0.5!(G)+(0,0.5*0.14583*\sT)$){$+$};
					\node[above,font=\tiny] at ($(C)+(0.09,0.5625*\sT)$){$\tfrac{9}{16}$};
					\node[below,font=\tiny] at ($(D)+(0,-0.9375*\sT)$){$\tfrac{15}{16}$};
					\node[above,font=\tiny] at ($(D)!0.62!(E)+(0,0.28125*\sT)$){$\tfrac{9}{32}$};
					\node[below,font=\tiny] at ($(F)+(0,-0.71875*\sT)$){$\tfrac{23}{32}$};
					\node[above,font=\tiny] at ($(F)!0.5!(G)+(0,0.14583*\sT)$){$\tfrac{7}{48}$};
					\node[font=\tiny] at (2,-0.92){(c) taglio $T$ \ ($\times\,p\ell$)};
				\end{scope}
				
				%==================== (d) MOMENTO M ====================
				\begin{scope}[shift={(0,0)}]
					\nodi
					\diagPar{C}{D}{0}{-1}{0}{0.140625}{-0.28125}{\sM}
					\diagLin{D}{E}{0}{-1}{-0.28125}{0}{\sM}
					\diagPar{E}{F}{0}{-1}{0}{0.015625}{-0.21875}{\sM}
					\diagLin{F}{G}{0}{-1}{-0.21875}{0}{\sM}
					\assi
					\node[above, xshift=16pt, yshift=8pt,font=\tiny] at ($(C)+(0.15,-0.16*\sM)$){$\tfrac{81}{512}p\ell^2$};
					\node[above,font=\tiny] at ($(D)+(0,0.28125*\sM)$){$\tfrac{9}{32}p\ell^2$};
					\node[above,font=\tiny] at ($(F)+(0,0.21875*\sM)$){$\tfrac{7}{32}p\ell^2$};
					\node[font=\tiny] at (2,-0.92){(d) momento flettente $M$ (fibre tese)};
				\end{scope}
		\end{tikzpicture}}
		\caption{Caso (c), carico asimmetrico ($p$ su $CD$ ed $EF$) --- (a) schema con carichi e reazioni; (b) sforzo normale $N$ sulle travi
			(le bielle, scariche, portano $N$ costante: $N_{AC}=-\tfrac{9\sqrt5}{16}$,
			$N_{AD}=-\tfrac{39\sqrt{17}}{32}$, $N_{BF}=-\tfrac{83\sqrt{17}}{96}$, $N_{BG}=+\tfrac{7\sqrt5}{48}$,
			in $p\ell$); (c) taglio $T$; (d) momento flettente $M$, dal lato delle fibre tese. Scale indipendenti;
			$M=0$ alle cerniere $C$, $E$, $G$.}
		\label{fig:stampella_NTM_c}
	\end{figure}
	
	\medskip\noindent\textbf{Lettura.}
	Venuta meno la simmetria, la chiave $E$ trasmette anche taglio
	($V_E\neq0$) e i segni si mescolano: una sola biella (la $BG$, dal lato meno
	caricato) risulta tesa. Lo scenario asimmetrico conferma che il
	funzionamento --- quali membri tirino e quali puntonino --- dipende dal
	carico, non dalla sola struttura.
	...
	\medskip\noindent\textbf{Sintesi: il segno al nodo $A$.}
	I tre scenari, sulla stessa identica struttura, danno funzionamenti diversi:
	in (a) i tratti d'estremità $CD$, $FG$ sono tesi e le bielle $AC$, $BG$
	compresse; in (b) l'impalcato è interamente compresso e le stesse bielle si
	invertono in tiranti; in (c), asimmetrico, i segni si mescolano. La chiave di
	lettura è l'equilibrio del nodo $A$ (Figura~\ref{fig:stampella_ventaglio}).
	La reazione di base $\mathbf r_A$ va scomposta lungo le due bielle: entrambe
	sono puntoni finché $\mathbf r_A$ cade \emph{dentro} il ventaglio delle
	direzioni $AC$, $AD$ --- il triangolo $A$--$C$--$D$, cioè l'anello a tre
	cerniere d'imposta. Quando $\mathbf r_A$ si fa più piatta della biella interna
	$AD$ ed esce dal ventaglio oltre $AD$, la componente su $AC$ inverte e la
	biella esterna diventa tirante:
	\[
	N_{AC}>0 \iff \frac{H_A}{V_A} > \frac{\beta}{f},
	\]
	dove $\beta=\overline{AD}_x$ è lo sbalzo orizzontale della biella interna e
	$f=\ell/4$ il ribassamento. Poiché il tratto d'estremità ha segno
	\emph{opposto} alla propria biella, e poiché la biella interna $AD$ è
	\emph{sempre} compressa mentre il corrente centrale porta \emph{sempre}
	$-H_E$, l'intera questione si riduce a quell'unico ribaltamento:
	\[
	\text{impalcato compresso}\ \iff\ CD\ \text{compresso}\ \iff\ AC\ \text{tesa}\ \iff\ \frac{H_A}{V_A}>\frac{\beta}{f}.
	\]
	Le forze disegnate nei poligoni sono quelle che le bielle esercitano \emph{sul
	nodo}; gli sforzi assiali nelle aste ne sono le reazioni, uguali e opposte
	(una freccia rivolta verso $A$ corrisponde a un puntone, una rivolta via da
	$A$ a un tirante).
	...
	\medskip\noindent
	I due funzionamenti estremi si leggono a colpo d'occhio nella
	Figura~\ref{fig:stampella_funzionamento}, dove lo spessore del tratto distingue
	la compressione (puntone) dalla trazione (tirante).
...
%======================================================================================================
%  ESERCIZIO 2 -- Conformazione geometrica per un impalcato compresso
%======================================================================================================
\begin{esercizio}[Conformazione geometrica per un impalcato compresso]
	\label{ex:stampella_progetto}
	
	Finora la struttura era \emph{data} e se ne studiavano le azioni. Rovesciamo l'impostazione: fissiamo
	una \textit{prestazione} e cerchiamo la \emph{geometria} che la realizza. È un caso particolare di un
	principio generale degli isostatici: la sola statica basta, in situazioni favorevoli, a tradurre un
	requisito di comportamento in un vincolo dimensionale, senza chiamare in causa rigidezze né materiali.
	
	\medskip\noindent\textbf{Passo~0 --- Prestazione richiesta.}
	Sotto peso proprio --- carico $p$ uniforme su tutto l'impalcato $CG$ --- si vuole che l'impalcato sia
	\emph{interamente compresso}. Per la sintesi del segno del nodo $A$, ciò equivale a $N_{AC}>0$ (bielle
	d'estremità $AC$, $BG$ \emph{tese}): basta imporre questa singola condizione.
	
	\medskip\noindent\textbf{Passo~1 --- Modello e parametri di progetto.}
	Schema ad arco a tre cerniere $A$--$E$--$B$ (\FIGref{fig:stampella_param}). Sono \emph{dati funzionali}
	la luce degli appoggi $L=\overline{AB}$, il ribassamento $f$ (quota dell'impalcato sopra gli appoggi) e
	lo sbalzo esterno $\alpha=\overline{AC}_x$; è \emph{incognita di progetto} lo sbalzo interno
	$\beta=\overline{AD}_x$, ossia la luce del tratto centrale $\overline{DF}=L-2\beta$. In coordinate
	$A=(0,0)$, $B=(L,0)$, $C=(-\alpha,f)$, $D=(\beta,f)$, $E=(\tfrac L2,f)$, $F=(L-\beta,f)$, $G=(L+\alpha,f)$,
	con bielle di lunghezza $\ell_C=\sqrt{\alpha^2+f^2}$, $\ell_D=\sqrt{\beta^2+f^2}$.
	
	%---------------------------------------------------------------- FIGURA: parametri geometrici
	\begin{figure}[htbp]
		\centering
		\newcommand{\cerniera}[1]{%
			\draw[terra] (#1) -- ++(-0.10,-0.18) -- ++(0.20,0) -- cycle;
	...
	\medskip\noindent\textbf{Passo~2 --- Reazioni esterne (carico $p$ su $CG$).}
	Tre equazioni cardinali e la cerniera in chiave $M_E=0$. La risultante verticale è
	$R=p\,\overline{CG}=p(L+2\alpha)$, simmetrica rispetto a $E$, dunque $V_A=V_B=\tfrac12 p(L+2\alpha)$; in
	assenza di carichi orizzontali $H_A=-H_B\equiv H$. Annullando il momento in $E$ sul semiarco di sinistra,
	\[
	M_E^{0}=\frac{p(L+2\alpha)L}{4}-p\Big(\frac{L^2}{8}+\frac{L\alpha}{2}+\frac{\alpha^2}{2}\Big)=\frac{p\,(L^2-4\alpha^2)}{8},
	\]
	equilibrato dalla spinta col braccio $f$:
	\[
	H=\frac{M_E^{0}}{f}=\frac{p\,(L-2\alpha)(L+2\alpha)}{8f},\qquad\text{onde}\qquad \frac{H}{V_A}=\frac{L-2\alpha}{4f}.
	\]
	Si noti che $H$ \emph{non dipende da $\beta$}: lo sbalzo interno non altera la spinta, ma --- come ora si
	vede --- ne governa la \emph{ripartizione} fra le bielle.
	
	\medskip\noindent\textbf{Passo~3 --- Equilibrio del nodo $A$ e azioni nelle bielle.}
	Con i versori $\mathbf a_{AC}=\tfrac1{\ell_C}(-\alpha,f)$ e $\mathbf a_{AD}=\tfrac1{\ell_D}(\beta,f)$
	(bielle assunte tese), le componenti si legano per $H_{(\cdot)}=\tfrac{a_x}{a_y}V_{(\cdot)}$ e l'equilibrio
	del piede dà
	\[
	\begin{cases} H-\dfrac{\alpha}{f}V_{AC}+\dfrac{\beta}{f}V_{AD}=0\\[6pt] V_A+V_{AC}+V_{AD}=0\end{cases}
	\;\Longrightarrow\;
	V_{AC}=\frac{Hf-\beta V_A}{\alpha+\beta},\qquad V_{AD}=-\frac{Hf+\alpha V_A}{\alpha+\beta}.
	\]
	Essendo $N=V/a_y$ con $a_y=f/\ell>0$, lo sforzo normale ha il segno della componente verticale: $V_{AD}<0$
	\emph{sempre} (numeratore positivo), cioè la biella interna $AD$ è sempre un puntone. Sostituendo
	$H$ e $V_A$ del Passo~2, la verticale dell'esterna si fattorizza:
	\[
	V_{AC}=\frac{p\,(L+2\alpha)\,(L-2\alpha-4\beta)}{8\,(\alpha+\beta)}
	\qquad\Longrightarrow\qquad \operatorname{sgn}N_{AC}=\operatorname{sgn}\,(L-2\alpha-4\beta).
	\]
	
	\medskip\noindent\textbf{Passo~4 --- Condizione di progetto e dominio ammissibile.}
	La prestazione $N_{AC}>0$ diventa una disuguaglianza \emph{puramente geometrica}:
	\[
	\beta<\frac{L-2\alpha}{4}\qquad\Longleftrightarrow\qquad \overline{DF}=L-2\beta>\frac{L+2\alpha}{2}=\tfrac12\,\overline{CG}.
	\]
	Il tratto centrale deve superare \emph{metà} della luce dell'impalcato. Il dominio è non vuoto purché
	$\alpha<L/2$ (sbalzi esterni non eccessivi); il ribassamento $f$ \emph{non} compare: pur incidendo
	sull'intensità delle azioni, non controlla il segno di $N_{AC}$ --- un risultato non ovvio, e una
	``trappola'' progettuale da tenere presente. Per il carico centrale, invece, la condizione si scrive
	$(L+2\beta)/4>\beta\Leftrightarrow L>2\beta$ ed è \emph{sempre} verificata: lo scenario di mezzeria è
	automaticamente favorevole, sicché la sola disuguaglianza su $\beta$ garantisce il comportamento voluto
	per entrambi gli scenari (\FIGref{fig:stampella_commutazione}).
	
	%---------------------------------------------------------------- FIGURA: commutazione del segno
	\begin{figure}[htbp]
		\centering
		\resizebox{0.62\linewidth}{!}{%
			\begin{tikzpicture}[x=3.2cm,y=1.05cm,font=\footnotesize,
				ax/.style={-{Stealth[length=2mm]},line width=0.6pt,black},
				cg/.style={line width=1.3pt,black},
				df/.style={line width=1.3pt,black,dash pattern=on 5pt off 2.5pt},
				sg/.style={line width=0.9pt,black!55,dash pattern=on 2.5pt off 2pt},
				gd/.style={gray!20,line width=0.3pt}]
				\def\xmax{2.1}\def\ymax{8.6}
				\foreach \bx in {0,0.25,...,2.0}\draw[gd](\bx,0)--(\bx,\ymax);
				\foreach \yy in {0,2,...,8}\draw[gd](0,\yy)--(\xmax,\yy);
				\draw[ax](0,0)--(\xmax,0) node[right]{$\beta/\ell$};
				\draw[ax](0,0)--(0,\ymax) node[above]{$\dfrac{H}{V_A}\,,\ \dfrac{\beta}{f}$};
				\foreach \bx in {0.5,1,1.5,2}\draw(\bx,0.08)--(\bx,-0.08) node[below]{$\bx$};
				\foreach \yy in {2,4,6,8}\draw(0.02,\yy)--(-0.02,\yy) node[left]{$\yy$};
				\draw[sg](0,0)--(\xmax,{4*\xmax});
				\node[black!55,anchor=north west,font=\scriptsize] at (1.30,4.6){soglia $\beta/f=4\beta/\ell$};
				\draw[cg](0,3)--(\xmax,3);
				\node[anchor=west] at (2.12,3){$\big(\tfrac{H}{V_A}\big)_{CG}=3$};
				\draw[df](0,4)--(\xmax,{4+2*\xmax});
				\node[anchor=south west,font=\scriptsize] at (0.40,6.50){$\big(\tfrac{H}{V_A}\big)_{DF}=4+2\tfrac{\beta}{\ell}$};
				\draw[gray!60,dashed,line width=0.4pt](0.75,0)--(0.75,3);
				\fill[black](0.75,3)circle(1.3pt);
				\node[below,gray!70,font=\tiny] at (0.75,-0.02){$\tfrac34$};
				\node[below,font=\tiny] at (0.78,3){$\beta^\ast$};
				\node[font=\scriptsize] at (0.40,7.9){$AC$ \emph{tesa}};
				\node[font=\scriptsize] at (0.62,7.55){(impalcato compresso)};
				\node[font=\scriptsize,align=center] at (1.62,1.3){$AC$ compressa};
				\fill[black](0.5,3)circle(1.0pt); 
				\node[font=\tiny,above=1pt,align=center]at(0.40,3){progetto\\($\beta=\ell/2$)};
				\fill[black](1.0,3)circle(1.0pt); \node[font=\tiny,below right=-1pt]at(1.0,3){originale, ($\beta=\ell$)};
		\end{tikzpicture}}
		\caption{Commutazione del segno di $N_{AC}$ al variare dello sbalzo interno $\beta$
			($L=4\ell$, $\alpha=\ell/2$, $f=\ell/4$). Per ciascuno scenario $AC$ è \emph{tesa} dove la curva
			$H/V_A$ sta \emph{sopra} la retta soglia $\beta/f$. Il peso proprio ($CG$, costante $=3$) supera la
			soglia solo per $\beta<\beta^\ast=\tfrac34\ell$; il carico centrale ($DF$) la supera sempre. Il
			dimensionamento $\beta=\tfrac12\ell$ cade nella regione favorevole, quello originario $\beta=\ell$ no.}
		\label{fig:stampella_commutazione}
	\end{figure}
	
	\medskip\noindent\textbf{Passo~5 --- Dimensionamento e verifica.}
	Con i dati dell'Esercizio precedente $L=4\ell$, $\alpha=\ell/2$, $f=\ell/4$, la soglia è $\beta<\tfrac34\ell$
	(ossia $\overline{DF}>\tfrac52\ell$). Il valore originario $\beta=\ell$ non la rispetta --- ed è proprio per
	questo che là $AC$, $BG$ erano compresse. Scegliamo allora $\beta=\tfrac12\ell\;(<\tfrac34\ell)$, da cui
	$\overline{DF}=3\ell>\tfrac52\ell$; con $\beta=\alpha=\tfrac\ell2$ le due bielle hanno egual pendenza
	($\ell_C=\ell_D=\tfrac{\sqrt5}{4}\ell$). Ricalcolando le azioni per i due scenari (in $p\ell$):
	\[
	\begin{array}{l|cccc}
		\text{carico} & N_{AC}=N_{BG} & N_{AD}=N_{BF} & N_{CD}=N_{FG} & N_{DE}=N_{EF}\\\hline
		\text{peso proprio } (CG) & +\tfrac{5\sqrt5}{8} & -\tfrac{25\sqrt5}{8} & -\tfrac54 & -\tfrac{15}{2}\\[2pt]
		\text{centrale } (DF) & +\tfrac{9\sqrt5}{8} & -\tfrac{21\sqrt5}{8} & -\tfrac94 & -\tfrac{15}{2}
	\end{array}
	\]
	In \emph{entrambi} i casi l'impalcato è interamente compresso (tratti d'estremità e corrente centrale
	negativi) e le bielle d'estremità $AC$, $BG$ sono tese: la prestazione è soddisfatta. La verifica sul
	carico centrale era del resto automatica, essendo $\big(\tfrac{H}{V_A}\big)_{DF}>\beta/f$ sempre.
	
	\medskip\noindent\emph{Lettura progettuale.} Imporre $\overline{DF}>\tfrac12\overline{CG}$ significa
	``raccogliere'' i piedi delle bielle interne sotto la mezzeria, lasciando lunghi i due correnti compressi:
	la spinta arriva alle imposte così piatta da uscire dal ventaglio delle bielle, e le esterne si invertono
	in tiranti. È la morfologia dei viadotti a cavalletto con campata centrale assai più lunga della base dei
	sostegni.
	
	\begin{oss}[\textbf{Un riscontro reale: il viadotto di Corso Francia}]
		Lo stesso principio si ritrova nei viadotti a travata sostenuta da telai a cavalletto. Nel viadotto di
		Corso Francia a Roma (R.~Morandi) la luce centrale è assai maggiore della distanza fra i piedi inclinati
		dei cavalletti --- nel nostro linguaggio, un tratto centrale $\overline{DF}$ ben oltre metà della luce
		dell'impalcato $\overline{CG}$, con sbalzi esterni $\alpha$ contenuti e bielle interne ripide ($\beta$
		piccolo): è precisamente la condizione $\overline{DF}>\tfrac12\overline{CG}$ del criterio. Sotto peso
		proprio l'impalcato risulta interamente compresso e le aste \emph{esterne} del cavalletto lavorano a
		trazione, come suggerisce anche il loro disegno snello a fronte dei puntoni interni massicci. Il
		confronto è naturalmente \emph{qualitativo}: la struttura reale è in cemento armato precompresso, con
		impalcato a cassone dotato di rigidezza flessionale propria e grado di iperstaticità non nullo ---
		aspetti che il modello isostatico ad arco a tre cerniere non coglie --- ma il \emph{segno} delle azioni
		e il ruolo dei parametri geometrici trovano riscontro.
	\end{oss}
